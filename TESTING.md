...
**Speed:** ~2 minutes for all 381 tests.
...
| `tests/candidate.spec.js` | Profile header (including `#race-context` DOM presence, `.tag-context` flex structure with `.tag-context-text` and link, `#compact-header` present and initially hidden, compact header child spans), cycle switcher as `select#cycle-switcher` with options, stats row (non-$0 financials), health banner, `#summary-strip` persistence across Summary/Raised/Spent tabs, first stat card label = "Raised-to-Spent Ratio", chart canvas, tab navigation, committees modal, Amplitude events, URL hash pre-selection, API correctness (no 422 errors) |
