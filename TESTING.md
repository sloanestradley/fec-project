...
**Speed:** ~1 minute for all 320 tests.
...
| File | What's covered |
|------|----------------|
| `tests/shared.spec.js` | Shared checks for every page: `styles.css` linked, `main.js` linked, top nav present with 3 links (Candidates/Committees/Races), correct active nav link, mobile search toggle present, warm parchment background, `Page Viewed` Amplitude event fires, nav logo `.logo-fec`/`.logo-ledger` spans present, `.global-banner` precedes `.top-nav` in DOM, desktop nav search wrapped in `.search-field` with icon, desktop nav search submit button is `.sr-only` with `aria-label="Search"` |
