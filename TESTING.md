...
**Speed:** ~1 minute for all 234 tests.
...
| `tests/pages.spec.js` | committee.html, races.html (page header, filter bar fields, office options, state combo, results area containers), race.html (candidate cards, financial figures, cycle-anchored links), candidates.html (auto-load, search input, clean URLs, filter chips, URL sync, error state, ?q= search mode, #load-more-spinner and #end-of-results DOM presence), committees.html (same as candidates), process-log.html, design-system.html (token tables, color swatches, component card attributes), index.html redirect, mobile layout at 390px (top-nav visible, search toggle visible) and 1280px (search toggle hidden) |
...
| Data fetching on races.html | Browse page structure built; fetch + rendering in follow-up |
