...
---
2026-04-21 (session 6) — Top Committee Contributors: distinguish empty-state copy by cause

## Process log draft

**Title: The detection signal that looked right and wasn't**

The Top Committee Contributors empty state on committee.html had been telling the same lie to two different committee classes: "Unable to show due to high transaction volume." For genuinely high-volume filers, accurate. For conduits (ActBlue, WinRed) and party committees whose inbound flows structurally aren't in pas2, it implied data that didn't exist. The Session 5 backfill had just made that contrast sharper — every smaller committee now had complete historical data back to 1980, which made the mega-committee empty state read like a gap rather than a feature.

The fix plan looked tidy. Two orthogonal detection signals, both computable from what `fetchRaisedData()` already had in hand: party committees via `committee_type`, conduits via the memo-rate on Schedule A page 1. Live-verified the committee types up front (DCCC is 'Y', ActBlue is 'V'), so the party signal would clearly work; the conduit signal was supposedly "overwhelming memo share on page 1" — easy heuristic. Planned it, approved it, shipped it, deployed it. ActBlue still rendered the old copy. DNC rendered the new one.

The miss was subtle but total: the Schedule A query was sorted `-contribution_receipt_amount` (for the aggregation itself), and ActBlue's top 100 rows by amount are the handful of large non-memo'd transactions — campaign principal committees receiving money back, ActBlue Technical Services internal flows — sitting above a very long tail of memo'd individual donations. Page 1 by amount was 0% memo. The full dataset is ~99% memo. The sample was biased exactly against what it was supposed to detect.

Swapped the conduit signal for `commPag.count > 500000` — total row count rather than memo-rate sample. That worked on the first try: ActBlue, WinRed, and DNC all render the new coverage-gap copy; Marie for Congress still shows real bulk-backed data; DCCC and RNC sit under PAGE_THRESHOLD anyway and are unaffected. A small two-word diff in the plan — "memo-rate" to "row count" — would have shipped the right fix first.

**Scope landed (expanded mid-session from the originally-planned Top Committee Contributors only):**

After the initial Top Committee Contributors fix shipped and was live-verified, the "What to bring to Claude Chat" block flagged two related follow-ups on the same page. Reviewed both, narrowed from both down to one: the Top Individual Contributors extension was dropped on review (individual-contribution data DOES exist in our source and in KV for conduits/parties — the rare KV miss + too-large-to-paginate state is an infrastructure gap, not a structural pas2 gap, and "Unable to show due to high transaction volume." is accurate for that). The Top Conduit Sources surface on conduit committees is a different thing entirely: asking "who conducted money through a conduit to this committee?" is semantically meaningless on a conduit itself, so the card is hidden rather than relabeled.

**Changelog:**
- committee.html: new `COMM_TYPE` global (parallel to existing `COMM_PARTY`), set in `renderHeader()` from `c.committee_type`.
- committee.html `fetchRaisedData()`: new `topCommitteesPas2Gap` and `topCommitteesIsConduit` booleans computed inside the over-threshold branch from `isParty = COMM_TYPE in {'X','Y'}` plus `isConduit = commPag.count > 500000`. `topCommitteesPas2Gap = isParty || isConduit`; `topCommitteesIsConduit = isConduit` (used separately for the conduit-card hide).
- committee.html `renderRaisedIfReady()`: (a) three-way branch on `topCommitteesSource` — gap copy ("Committee contribution data is not available for this committee type."), high-volume copy (unchanged), or real table. (b) conduit-card render now has a hide-first branch: `isConduit` → `#conduits-card` hidden entirely (`display:none`); else the existing too-large → empty-copy → real-rows three-way. (c) raised-tab data-note suppresses its "Top conduit sources: aggregated from memo entries..." sentence when the card is hidden.
- **Top Individual Contributors NOT extended** (decision made mid-session — flagged as a follow-up in this log, not delivered here).
- CLAUDE.md architectural-debt entry rewritten to describe both treatments (gap copy for Top Committee Contributors, card-hide for Top Conduit Sources), the memo-rate failure mode, and the explicit non-extension of Top Individual Contributors.
- test-cases.md: six new manual test cases across the two surfaces (three for the gap-copy + high-volume + happy paths on Top Committee Contributors, three for the hide + high-volume + happy paths on Top Conduit Sources); single log-row updated in place rather than adding a second.
- 417/417 Playwright tests passing throughout; no test changes required (existing unavailable-state test at `pages.spec.js:271` asserts only the individual-contributors tbody, which this session didn't modify).
- Live verification on fecledger.pages.dev: ActBlue + WinRed render gap copy on Top Committee Contributors AND have `#conduits-card` hidden; DNC renders gap copy on Top Committee Contributors AND the existing "Unable to show..." copy on its visible `#conduits-card`; Marie renders real bulk-backed data on Top Committee Contributors and the existing high-volume copy on her visible `#conduits-card` (pre-existing pagination-gate behavior; she's over the 100-page gate but under the 500k conduit threshold, so the hide branch doesn't fire); DCCC / RNC unaffected (< PAGE_THRESHOLD).

**Field notes:**

One lesson is about the detection signal. Memo-rate on page 1 seemed obviously correct — conduits are memo-heavy, so check whether page 1 is memo-heavy. The bug wasn't in the reasoning but in the sample. Sorting page 1 by amount pulled from a distribution slice where conduits look most like normal committees. The fix (`count > 500000`) works because it samples the population, not the top of the distribution. When the data you're sampling is going to be used as a signal about the whole, check whether the sort itself biases against the thing you're trying to detect. This would have been catchable in the plan with one explicit sentence: "sort by `-contribution_receipt_amount` biases toward which kind of row?" The lesson isn't about planning more — it's about the specific discipline of checking sample bias when a detection signal samples a subset.

A second lesson: verifying the committee types up front (curl on ActBlue and DCCC before writing code) was the right move for the party signal, but it gave the conduit signal a false sense of completeness. I verified what was true (`committee_type === 'V'` for ActBlue), and derived from that "committee_type alone doesn't distinguish ActBlue from non-conduit Hybrid PACs" — correct. But I didn't curl the Schedule A page 1 data to verify the memo-rate claim itself. The verification pattern was asymmetric: I validated the evidence for the rejected approach but not the evidence for the approved one. A five-minute curl (what I did after the fact to diagnose) would have caught this at plan time.

Third: the pre-deploy curl on DCCC's KV state was Sloane's call, and it was valuable not because of what it confirmed but because of what it disconfirmed. The plan's verification step had said "DCCC shows real KV data." The curl proved DCCC's top-committees KV was a miss. Sloane made the plan explicit ("state the expected outcome explicitly rather than discover it during testing") and that forced the check, which surfaced the mis-stated expectation. DCCC still rendered correctly in the end — not via the KV path I'd assumed, but via the under-threshold aggregation path that my change doesn't touch. The expected outcome ended up right for a different reason than I'd written. Naming the expected outcome was what forced the diagnosis.

**Stack tags:** Detection sampling bias · sort-order × sample-representativeness · committee_type X/Y party signal · commPag.count conduit signal · post-deploy live verification via Playwright

## How Sloane steered the work

**"Before building, do a quick curl on top_committees:C00000935:2024 in the KV namespace to confirm whether DCCC has a KV entry."**

This was the single highest-leverage correction of the session. The plan's verification step said DCCC would show real KV-backed data (an assumption I'd written without checking), and Sloane flagged that the step should "state the expected outcome explicitly rather than discover it during testing." The curl proved DCCC's top-committees KV is a miss — which meant the plan's verification rationale was wrong, even though DCCC's rendered outcome happens to be fine (it doesn't hit PAGE_THRESHOLD, so it aggregates live). The lesson is the same one you've pushed in past sessions: plan prose that asserts an outcome has to resolve to a checkable claim, not a plausible guess. Without that ten-minute check, I'd have shipped a plan whose verification section described a committee's state-space transition that didn't actually happen.

**Picking the "keep current copy" option for the high-volume case.**

The three options I offered for the high-volume empty state each made a different implicit bet: refine it, keep it, or commit to a future-tense promise. You picked "keep current" over both "refine slightly" (cosmetic change) and "promise future data" (a commitment we don't have a concrete plan to deliver). That's the same pattern I keep seeing: when the right-hand side of a tradeoff is a promise we'd have to keep later, don't write it. The copy distinction alone does the work of making the other empty state honest — no accessory copy needed.

**Strict scope: Top Committee Contributors only — initially.**

You originally picked the narrower scope over extending the treatment to Top Individual Contributors and Top Conduit Sources. Those two surfaces also carried the same misleading copy on the same committees, and extending would have produced a more visually consistent story. Later in the session, after the Top Committee Contributors change was live, you opened the follow-up and asked me to review both surfaces before acting. That review produced a real second narrowing: Top Individual Contributors was dropped entirely (the data DOES exist — rare KV miss + too-large-to-paginate is an infrastructure gap, not a structural one; the existing copy is accurate for what it is), and Top Conduit Sources was reframed from copy change to card-hide (asking "who conducted money through a conduit to this committee?" is semantically meaningless on a conduit). Both narrowings came from reading each surface's actual semantics, not from scope-cutting for its own sake.

The broader pattern: you ask for a review before extending a fix, then let the review change the shape of the fix. That's why the extension shipped tighter than it would have otherwise — a hide, not a relabel; one surface, not two.

**The through-line:** every steering moment this session was about forcing assumptions into checkable claims or explicit scope. The verification-rationale curl turned "DCCC has KV" from plan-prose into "let me see the response." The copy choice refused a future-tense bet that would have been unfalsifiable. The scope limit kept this session one clean shippable thing rather than three loosely-related ones. None of these were large redirects; all of them were the difference between "done" and "done right."

## What to bring to Claude Chat

- **The conduit threshold (500k) is empirically set, not principled.** ActBlue (~11M) and WinRed (~5M) sit well above it; no known legitimate non-party committee in the 100k-500k range was identified. But "no known" isn't "none" — a future platform in the 200k range would be mis-classified as "high volume" rather than "coverage gap" until it grows past 500k. The threshold is load-bearing in a soft way. Worth periodically checking `commPag.count` distributions for newly flagged committees to catch drift.

- **The memo-rate signal is still a tempting design pattern for a future bulk-data project** — e.g. if we ever ingest Schedule A memo rows into a separate bulk source, memo-rate becomes a first-class property of a committee, not a sampled heuristic. Worth keeping in mind when scoping the next bulk-data feature; the signal isn't wrong conceptually, only wrong as a biased-sample detector.
...
T9 + T10 + verification fixes — The two-page helper, the scrollTo-clamp pattern returns, and a regression-parity sweep

The committee.html parity arc that opened with T8 (index landing state, All-time mode removed) closed in this session. T9 lifted candidate.html's switchView() into a reusable initViewSwitcher() factory in utils.js. T10 made committee.html consume it, retiring the T8-era window.location.reload() patch. Manual browser verification then surfaced four distinct bugs across three commits, all now fixed. Both profile pages share the same transition machinery; the helper is the standard mechanism going forward.

Six commits this session:
- `8ec232b` T9 — Lift switchView() to initViewSwitcher() in utils.js
- `0eef3df` T10 — committee.html in-place transitions, helper adoption
- `b9761d2` Documentation pass (later updated in place — see workflow note below)
- `3f30b62` Fix: minHeight-as-floor scroll-clamp on sync loadCycle wrap
- `c31d58d` Fix: trailing slash on committee URL
- `71236a0` Fix: no-ID error state + invalid tab + non-existent year (parity sweep, surfaced latent candidate bug)

What you have now: both profile pages share the same transition machinery. Click a cycle row, the profile header doesn't reflow — only the content below it swaps. Browser back restores the index from cache without re-fetching. Compact-engaged scroll position carries through transitions correctly on both pages. Trailing-slash URLs resolve. No-ID URLs surface friendly Browse links. Invalid hashes fall through to safe defaults.

Test count: 496/496 Track 1 passing (476 baseline + 20 new tests this session — 8 in-place transition + 4 path-segment URL × 2 pages + 4 parity-sweep × 2 pages, with one shared/dedup).

Field notes:

The most interesting part of the planned implementation was committee's renderStats. Candidate's loadCycle is async — it awaits Promise.all on totals fetches. Committee's renderStats is synchronous — totals are pre-fetched into ALL_TOTALS at init, summary stats render from a lookup. So when the prompt said "loadCycle is a sync wrap — Promise.resolve(renderStats(cycle))", I built that wrap, and then realized the token mechanism needed a different home. Renderstats's "race surface" isn't the summary writes (those finish before the function returns). It's the raised/spent fetches that renderStats kicks off and stores in raisedDataPromise/spentDataPromise — the renderXIfReady functions await those promises and write DOM. Without a token check there, rapid cycle clicks let stale Schedule A/B writes clobber fresh DOM. One module-scoped cycleFetchToken claimed at the top of renderStats and checked in both renderXIfReady functions did the job. Single token covers both raised+spent because both belong to the same cycle change.

**The scrollTo-clamp class of bug — second instance documented.** The first instance was T6.5 on candidate.html: the helper sets minHeight on .main to support targetScrollY, scrollTo lands correctly, then clears minHeight after loadCycle resolves — assuming loadCycle has grown the natural document tall enough. True for candidate's async loadCycle (awaits totals fetches; DOM grows during the await). False for committee's sync wrap (Promise.resolve(renderStats(cycle))) — detail content stays skeleton-short with raised/spent tabs in lazy-load state. Document collapses below targetScrollY+viewportH; browser silently clamps scrollY (often to 0); compact scroll listener subsequently disengages compact. **Two instances of the same shape now earn it formal documentation as a class of bug to watch for whenever a view transition targets `scrollY > 0`.** Resolution: don't clear minHeight at all. The minHeight is a harmless floor when natural content exceeds it (typical after raised/spent tab switches) and a necessary preservation when natural is short. A clear-then-measure-then-restore approach was tried first and failed — the momentary clear is enough to trigger the browser-side clamp, and restoring the value can't re-establish the lost scrollY. The diagnostic that confirmed the cause came from instrumenting initViewSwitcher with five console.log lines and writing a tiny one-off Playwright spec to capture them on both pages — side-by-side comparison of mainEl.scrollHeight before/after minHeight clear made the divergence obvious in three minutes. The diagnostic file was removed before commit; the instrumentation discipline is worth keeping (instrument → measure → diagnose → fix → remove logs → commit).

**Regression-parity verification surfaces latent bugs in unrelated pages.** When the trailing-slash bug landed on committee, the prompt asked for a parity sweep across four other candidate-era patterns (no-ID error state, invalid tab hash, non-existent year, plus the trailing-slash case). Three of the four checks confirmed clean on both pages. The fourth — no-ID — surfaced a latent bug on **candidate.html that wasn't part of the planned scope**: `/candidate` (no path segment) extracted `'candidate'` as CANDIDATE_ID, fired a 404 fetch, and surfaced a generic "Could not load candidate" error instead of the friendly "No candidate ID provided. Browse candidates →" branch. Nobody had reported it; the prompt only asked about committee parity. **Cross-page parity verification catches this class of dormant bug because it forces examination of both pages even when only one is broken.** Worth banking as a discovery technique: when fixing a bug that has a parallel page, sweep the parallel page's analogous code paths even if no one has reported a bug there.

The "0 existing tests needed updates" outcome surprised me. I'd planned to update one — the cycle-row-click test that previously assumed full-page reload. Turns out the assertion (wait for #summary-strip.visible, check URL) is true under both behaviors. The test is a survivor by good design — it asserts on observable end state, not implementation. The 8 new in-place tests fill in the previously-untested gaps (no-reload, DOM identity preserved, back-cache hit on metadata fetches, scroll behavior, compact continuity, rapid hash navigation).
...
Test scaffolding lesson: the original committee in-place-transition test used `body.minHeight=3000px` to scroll past compact threshold. That scaffolding masked the scroll-clamp bug for the entire T10 implementation phase — only manual browser verification caught it. Fix: switched to inflating cycle-index padding only, so detail content stays naturally short (the realistic flow). Migrated candidate.spec.js's analogous test in the post-verification ritual to use the same pattern, closing a third-instance gap.

Stack tags: initViewSwitcher factory pattern (mirrors initComboDropdown), single-counter token namespace exposed via API, per-page fetchIndexData cache lambda, sync loadCycle wrap with token-protected downstream renderXIfReady, compact-naive helper via callback decoupling, minHeight-as-floor pattern (don't clear after sync loadCycle), index-only-padding test scaffolding (vs body-inflation), regression-parity verification as a bug-discovery technique

## How Sloane steered the work

"Add an inline comment near the minHeight cleanup logic noting that the current behavior assumes loadCycle never rejects."
This came at the end of the T10 plan review. The minHeight-after-await pattern works today because loadCycle's internal try/catch never lets it reject — but that's an implicit contract, not an explicit one. (Resolved differently than expected — the entire minHeight-clearing logic was removed in the post-verification fix, so the comment got rewritten as the floor-pattern explanation. Same instinct paid off in a different shape.)
...
**End-of-session rituals run AFTER verification, not bundled with implementation.** The first ritual on this session shipped before manual browser verification surfaced four distinct bugs. Its field notes were forward-looking optimistic ("manual browser verification still outstanding"); the closing summary claimed the arc was complete. After verification, four fixes landed across three commits. The ritual entry has now been updated in place to reflect the completed reality — what's actually true post-debugging, not what was hoped at implementation time. **Generalizable pattern for future sessions: end-of-session rituals run AFTER verification. Pre-verification rituals are drafts, not records.** Saved as a feedback memory for future sessions to apply automatically.

"Regression-parity check before ritual" — Sloane's framing for the prompt that surfaced the latent candidate.html bug. The framing matters: not "fix this committee bug" but "verify the candidate-era patterns also apply on committee." That framing forced examination of candidate's analogous code path on the no-ID case, which turned up the latent bug. Direct framing matters at the bug-hunt level — same way "verify the data before writing the SQL" matters at the implementation level. Both are about not assuming the parallel side is correct just because it's not the page in front of you.

"Specific WHY sentence required for the never-cleared minHeight comment." Sloane's review of this ritual's CLAUDE.md plan caught that the floor pattern needed explicit protection from "well-meaning refactors." The added sentence ("Don't 'clean up' the never-cleared minHeight — it's load-bearing.") prevents a third-instance occurrence. Same instinct as the inline guard comments earlier in the session: defensive code deserves WHY documentation, especially when the WHY isn't visible from the call site.

The through-line: you steer for code AND processes that age well. The code instincts are about future-CC reading the helper without your context. The process instinct (rituals after verification) is about future-Sloane reading the ritual entries and trusting them as records, not drafts. Both apply the same discipline at different layers.
...
- **The minHeight-as-floor pattern is now documented as a class of bug.** Watch for it whenever a view transition targets scrollY > 0. The conservative answer is "don't clear at all"; the floor is harmless when natural content exceeds it and necessary when it doesn't. Documented in CLAUDE.md compact sticky header note (4) with Sloane's WHY sentence protecting against future "looks like dead code" refactors.
- **The "test the destination, not the transition mechanism" lesson.** T10's existing cycle-row-click test passed unchanged because it asserts on observable end state, not on whether a reload happened. Worth banking as a test-design pattern: assertions on final DOM state are robust across implementation refactors; assertions on URL state alone are fragile (the T8 close-out caught this when an assertion passed because URL changed even though the view didn't render). The lesson: test the destination, not the transition mechanism.
...
---
2026-05-14 — T14 + T14.5 stats-grid restructure + back-affordance migration, polish arc, T16 cycle-switcher retirement, T-bug tab-panel desync fix

## Process log draft

**Subtracting backwards from the stats-grid until the page held its own shape**

The session set out to ship T14 + T14.5 — restructure the stats-grid into a 1+2/2+3/1+3 layout and move the back-affordance into the Cycle card. Both shipped atomically as planned. What I didn't anticipate was how much polish would follow: the lift uncovered visual mismatches the prior masthead had been hiding, and each subsequent prompt was a smaller surgical edit than the one before — banner padding, race-context-bar gutter, mobile divider extent, ::after navy line on the tabs-bar, the negative margin on .tab that had been clipping the active-tab underline. By the time T16 retired the cycle switcher from the tabs-bar and the chevron + cycle-row path became the only cycle-change route, the layout language across stats-grid / banner / race-context-bar / cycle-rows / tabs-bar had collapsed to one consistent pattern: parent owns the gutter on desktop, bordered children sit inside with internal breathing room, mobile reverts edge-to-edge.

The session closed with a real production bug — tab-panel desync after the chevron + cycle-row round trip — and a clean fix that closed the class rather than pinning the exact root cause. Choosing to ship the bug fix BEFORE T16 (had the timing allowed it) felt right; the lesson is mine, the principle was yours.

Changelog:
- **T14 (4520ee0..f345445):** stats-grid restructured to outer 1fr 1fr + nested .stats-grid-half nests with per-context --cols (1/2 for index, 2/3 candidate detail, 1/3 committee detail). History card replaces First Filed + Last Activity on index views, sourced from /candidate/{id}/totals/.coverage_end_date (Option A — zero new API calls) and /committee/{id}/.last_file_date. Bare-year fallback when first cycle == last cycle. Stat-coverage card retires from committee detail; freshness signal moves to #stat-coh-sub's "As of {date}" line.
- **T14.5 (same commit):** Cycle card with inline back chevron replaces the masthead .back-affordance-slot. Click handler simplifies to always `replaceState + view.switchTo(false, NaN)` — wasIndexShown / indexShown / headerEl config / .detail-view class toggle all retire (the .detail-view class had no other consumer beyond the masthead chevron's CSS scope). UX trade documented: chevron isn't reachable in compact-scroll state — accepted, browser-back is the deep-scrolled escape valve.
- **Polish arc (44f79a8):** stat-cycle writes synchronously at the top of loadCycle so prior cycle's range doesn't sit during the await window. .page-title margin-top retired (masthead is ~48px shorter now). .stats-grid gains 16px margin-top so the navy top border doesn't slide flush under the compact sticky header.
- **Gutter-lift border pattern (44f79a8 + ba78ada):** strips own the gutter padding on desktop, bordered children sit inside the inset (.stats-grid, .banner, #race-context-bar, .cycle-index rows). Mobile reverts: strips zero their gutter, bordered children carry the gutter themselves so borders go edge-to-edge of the viewport. .stats-grid is intentionally edge-to-edge on mobile (cards' own 24px padding provides breathing room; mobile width is precious). Internal 16px padding added to .banner, #race-context-bar, cycle-rows, and .tabs-bar tabs for breathing room from their own border edges — .stats-grid excluded.
- **.tabs-bar bottom rule (ba78ada):** navy 2px border-bottom migrated to a ::after pseudo-element anchored at left/right:var(--page-gutter); z-index:-1 keeps the .tab.active red underline above the navy in stacking order. .tab's margin-bottom:-2px retired in the same change — it was clipping 2px of the red border under overflow-y:hidden.
- **#stat-ratio-sub static default retired (f8a7c04):** "Raised ÷ Spent" placeholder gone; sub stays blank when no contextual label (Healthy range / Low cushion / Overspend) applies. Matches the H.3 decision pattern from T14 on #stat-coh-sub.
- **T16 (1def54e):** in-tabs-bar cycle switcher retired on candidate.html + committee.html. Cycle changes now route through the Cycle card chevron → cycle index → row click path. Cycle Switched Amplitude event retires (replacement signal: Page Viewed { view: 'detail', cycle } via trackPageViewed on row-click hashchange). 7 cycle-switcher assertions retire from tests; 2 rewritten via the new path. .cycle-select CSS stays — race.html keeps the variant.
- **T-bug fix (89024a6):** moved restoreTab(tabHash || 'summary') from POST-await to PRE-await inside view.switchTo's detail branch. Closes a desync window where leaving + re-entering detail at a different cycle preserved stale tab-panel display state. Two new tests (one per page) lock the pre-await placement structurally.

Field notes:

The "investigate before scoping" discipline showed up six times this session — T14, T14.5, T16, the gutter-lift, the bug, even the doc-update question. Each diagnostic pass cost 20-30 minutes and the implementation that followed needed near-zero rework. By the end of the session, the diagnostic-first prompt is the default expectation, not an exception. The investment compounds.

The biggest structural pattern this session was the override → lift instinct, applied at every layer. T14 lifted the back-affordance from masthead to Cycle card. T14.5 follow-up lifted gutter padding from bordered children to strip wrappers. The .tabs-bar lift used a ::after pseudo-element to encode the gutter inset because the element was self-contained. Every time we hit an asymmetry between "this needs padding here" and "this needs padding there at a different breakpoint," the right answer was almost always "lift the property to where context dictates the value, and revert at the breakpoint where context inverts." The codebase ends each pass smaller in the directions that matter.

The T-bug fix is the structural-instead-of-causal lesson worth banking explicitly. The trace said showTab should run; the user observed it didn't. Rather than chase the exact race, I moved the restoreTab call to a synchronous pre-microtask-queue execution context — eliminating the window in which the inconsistency could appear, regardless of which variant of (a)/(b)/(c) was the actual cause. Sloane's framing — "the fix structurally closes the class of bugs that includes it" — captured the right engineering instinct. When the cost of pinpointing is high and the cost of closing-the-class is low, close the class.

The fix-first principle is the smaller but worth-banking lesson. My initial recommendation was "ship the fix ASAP after T16." Sloane flipped it: "fix-first, then T16." Three good reasons given (production correctness, T16 verification cleanliness, separate-concerns-separate-commits) — but I should have suggested the flip myself, given the bug was production-visible. Next time the trade comes up, fix-first is the default.

Stack tags: parent-owns-gutter / children-sit-inset CSS lift pattern · ::after navy line replacing border-bottom for self-contained sticky elements · synchronous-pre-await tab-panel reset closing async-window desync class · year-range derived from coverage_end_date instead of last_file_date (Option A — zero new API calls) · DuckDB-spec-irrelevant: pure UI session

## How Sloane steered the work

**The investigate-before-scoping discipline, applied seven times.**
Every multi-step ticket this session opened with a diagnostic-only prompt: T14 + T14.5 (large investigation, 5 deliberate decisions), gutter-lift on summary/career-strip (exploratory question → recommendation → confirm → implement), mobile-vs-desktop padding question (asked for assessment before implementing), .tabs-bar + #cycle-index lift (asked CC to "assess low-risk opportunity"), T16 removal investigation, T-bug investigation. Each diagnostic pass surfaced concerns I wouldn't have spotted otherwise (.cycle-select stays alive via race.html consumer; tab-context loss as a real UX trade; .candidate-card needing color:inherit when .tag-neutral lost its explicit color; .stats-grid not needing the internal-padding treatment the banner does). The discipline isn't aspirational anymore — it's the default cadence.

**Scope corrections, twice in the same arc.**
First on the gutter-lift internal padding: I'd assumed "bordered children" meant all four (stats-grid + banner + race-context-bar + cycle-index rows). You corrected: only banner + race-context-bar — stats-grid already has breathing room from .stat-card's 24px padding. Second, immediately after: mobile .stats-grid padding I'd just added based on the corrected scope — you removed it ("mobile, we could really buy back space"). Both corrections were about reading the visual situation more precisely than my categorical instinct did. Both saved real polish from being mis-shipped.

**"Frame A. Accept the regression."**
On tab-context loss across cycle changes post-T16 — current switcher preserved the tab, row-click path forces Summary. I'd flagged it as a real UX trade requiring a decision. You named the right call: "Frame A. Sloane noted the tabs themselves may not be here to stay in their current form; mitigating a regression on a UX surface that may evolve is premature." That's the discipline of accepting the cost when the surface might shift anyway — don't optimize around something you might redesign. Worth banking as a sequencing instinct, not just a single-decision call.

**Fix-first, then feature — corrected my sequencing.**
I recommended shipping the T-bug fix AFTER T16. You flipped it: fix-first, three reasons given. Production correctness independent of feature timing, T16 verification cleanliness, separate concerns / separate commits. Right call I should have made myself. The principle generalizes: when a bug surfaces during a feature's verification AND exists in production independent of that feature, fix-first is the cleaner sequencing. Banking.

**"Apply the same treatment elsewhere" — three times.**
After the gutter-lift on summary-strip / career-strip, you asked: "What if I do want the borders to extend full-width into the page gutter on mobile?" Then "Apply the same to #cycle-index and #tabs-bar?" Then "Add left/right padding to the content on desktop to mimic banner / race-context-bar?" Each was a follow-up that extended the pattern further. The cumulative effect was a consistent system across five elements (stats-grid, banner, race-context-bar, cycle-rows, tabs-bar) — which I would have left as one-off fixes on the original two. You saw the system before I did.

**"Investigation only, no code changes" framing on every cleanup.**
Each ticket-shaped prompt opened with explicit "no code changes" instruction before the diagnostic. That kept the investigation honest — I couldn't accidentally start implementing while still figuring out. The implementation prompts that followed had nothing to relitigate. Same pattern from prior sessions, applied uniformly here.

**The override → lift pattern, observed three more times this session.**
You didn't need to call it out — you just kept doing it. When the inline #summary-strip .stats-grid padding override felt fragile, you asked about lifting it. When .banner padding overrode the parent strip's gutter, you asked about moving the gutter up. When .stats-grid internal padding felt redundant against .stat-card's existing padding, you removed it. Three more instances of the same instinct from T21. Now four total this project. Worth a real memory.

**Mobile space as a first-class concern.**
"In mobile, it's causing the border between the two rows to be inset and adding unnecessary padding on mobile where we could really buy back space." You're treating mobile horizontal pixels as scarce — which is correct, and which I'd undervalued by trying to maintain visual symmetry. The mobile stats-grid going edge-to-edge with no horizontal padding is the right call on principle, not a deviation from the desktop system.

**Visual-QA-driven iteration loops.**
The chevron position. The .page-title margin. The stats-grid margin-top. The race-context-bar padding on mobile. The .tab clipped border. Each of these landed because you pushed the commit to production and then eyeballed it. The iteration loop — deploy, look, narrow surgical edit, repeat — is the right shape for design-system polish. Plan-on-paper can't reason about navy-line density or compact-header transitions; only the real page can.

**The through-line:** you steer for the version of the work that ages well. Every override that accumulated, you lifted. Every visual mismatch the masthead was hiding, you surfaced and fixed once the lift exposed it. Every "let's try this" iteration was scoped tight enough that it could be reverted independently. And when a bug surfaced in production, you didn't let the in-flight T-feature delay the fix — you sequenced it ahead. That's a designer's instinct showing up at the code level: maximize coherence, minimize accumulated friction, and ship the right thing in the right order.

## What to bring to Claude Chat

- **Post-deploy validation: tab-context loss on cycle change.** Frame A accepted the regression in this session. Real-world question: after using the post-T16 cycle-change flow for a few days, does the Summary-always landing actually feel disruptive when comparing a single tab across cycles? Worth a real "use it for a week, then revisit" cadence — if it does feel disruptive, the cycle-row tab-aware href is a small follow-up; if it doesn't, retire the question.

- **Cycle card chevron in compact-scroll state.** The chevron disappears with the strip when scrolled past — H.1 trade. Browser-back is the deep-scrolled escape valve. Worth real-world validation: do you find yourself ever needing to navigate to a different cycle from deep-scrolled tab content, or does the scroll-position-as-context-signal mean you always re-orient to top first?

- **`.tabs-bar` future ("may not be here to stay in their current form").** Sloane mentioned this in passing when accepting Frame A. Worth a real strategy conversation: what's the eventual shape of the tabs-bar? Sticky always? Retire altogether and inline the sections? The current investments (gutter-lift, ::after navy line, internal padding, T16's switcher retirement) are all consistent with the current shape — but if the shape's shifting, knowing the direction would inform whether further T-prefixed work on this surface is worth the investment.

- **The T-bug fix shipped without root-cause pinning.** The "structurally close the class of bugs" approach worked here because the trace was clean enough to know what window to close. The fix is robust regardless. But it's a precedent worth being deliberate about: when a similar production-bug-with-clean-trace-but-unclear-cause comes up next time, are we comfortable shipping the class-closing fix without pinning the root cause, or do we want to invest in a deeper diagnostic? My instinct after this session: class-closing is fine when the closure mechanism is clearly understood (here: synchronous-pre-await execution context). When the closure mechanism itself is ambiguous, dig.
...
---
2026-05-21 — IA cleanup + search-extension visual identity + terminated toggle inversion

## Process log draft

**Title:** What "search-extension page" actually means in practice

This session reframed three pages — /search, /candidates, /committees — from "destination pages with their own nav presence" to "search-extension surfaces reachable from the search results' View all affordances." The IA change was small in code (remove two nav links across 10 HTML files) but the conceptual shift forced 14 commits worth of follow-on work to make the framing visible in the pages themselves: tighter content column, no page-title affordance, a single full-width search field with the remaining filters wrapping below, chip restyle, form-control corner-radius, typeahead floating shadow. By the end, /candidates and /committees feel like extensions of Search instead of independent destinations — which was the goal.

A second arc shipped along the way: the terminated-committees toggle on /committees defaulted to excluding terminated, but cross-entity search returns committee counts that INCLUDE terminated. Anyone clicking "View all 10 →" from search landed on N-7 results. Inverting the toggle (default off = "exclude terminated" → terminated INCLUDED) closes that confusion. The fix touched one HTML file, the inversion took six lines of code, and the conversation about whether to flip the `?terminated=1` URL param semantic (Option A) or rename it (Option B) was the substantive part.

**Changelog (14 commits, three arcs):**
- T-IA-candidate-committees-nav-removal: Candidates + Committees out of top-nav and mobile drawer across 10 files; .nav-link.active / .nav-item.active CSS rules + markup applications retired entirely (were visual no-ops); 593 → 573 tests (net -20)
- Search-extension surface foundation (fe51bf6): .page-header retired on the three pages; .main-inner-narrow:800px modifier introduced; wrap horizontal padding restored to var(--page-gutter); 573 → 572
- Terminated toggle inversion (c10abbd): "Show terminated" → "Exclude terminated"; default OFF = include; URL param semantic flipped per Sloane Option A; CLAUDE.md DM Sans drift fixed inline; 572
- Search-bar consistency arc (2cc325b → 31d20b1): full-width input via .form-field-full modifier on /candidates and /committees, aria-label replaces visible "Search" label; .search-bar input flex:1 fix for latent intrinsic-width bug; row-vs-column filter-bar gap; form controls border-radius 2px; chip restyle; toggle-switch height:34px; typeahead floating shadow + 2px radius + min-height; max-height removed on the four search-typeahead instances
- Border architecture iteration: lifted wrap border-bottom → .results-area border-top → all-state-container border-top → reverted to original wrap border-bottom; chips-wrap permanently border-less (only deviation from pre-arc state)
- Closing cleanup (today): .filter-bar max-width:860 retired (never kicked in); .search-combo .form-input min-width 160 → 0; #state-empty wrapper removed from search.html; CLAUDE.md drift on min-width and .search-bar description corrected

**Field notes:**

The "search-extension page" framing felt obvious in conversation but had non-trivial downstream cost. Removing the .page-header was the obvious move; what wasn't obvious was that the full-width search input needs an entirely different layout posture (filters wrap to a second row), which then needs a new modifier class, which then surfaces a latent /search input-width bug, which then needs CSS, which then puts pressure on the typeahead width cap, which then surfaces the .typeahead-dropdown triple-duty problem... Each step was small but the chain was long. The session became an exercise in keeping the conceptual frame ("these are search-extension surfaces") visible while the implementation rabbit-holed.

The border architecture iteration was instructive. Tried three positions over four commits — bottom borders on wraps (original), top borders on .results-area, top borders on every state container, back to original. Each iteration solved one problem and introduced another (state flicker, layout shift, visual inconsistency). The revert to original wasn't a failure — it was the recognition that the original position was right for the wrong reason (and the iteration confirmed the right reason). The .filter-chips-wrap losing its border permanently is the only durable change; the chip row floats below the divider as a layer of optional state, which reads cleaner than a second divider would. Worth banking: "tried iteration, reverted to baseline" doesn't always mean wasted effort — sometimes it means you now understand why the baseline was right.

The latent /search input-width bug was a 4-month-old issue masked by the previous max-width:600px cap. When the cap was retired for the wider-input direction, the bug surfaced (~168px intrinsic input width inside an otherwise 704px container). The fix is one line, parallel to existing `.search-combo .form-input { flex:1 }`. The interesting thing is what kept it latent: the visual outcome was technically wrong but not obviously wrong, because the input was always at the LEFT edge of the wider container — empty space to the right read as "open layout" instead of "input not filling." Cap removal made it impossible to ignore.

The `.typeahead-dropdown` triple-duty problem was a banked refactor by end of session. Same class wraps search typeaheads (4 sites + nav), state-combo (3 sites), and click-combos (8+ sites). Three different interaction patterns. The session put it on the radar without resolving it — the right call given today's scope, but a real piece of accumulated drift worth its own ticket.

The cold-read audit at end-of-session caught two drift items in CLAUDE.md (`.search-combo .form-input min-width:160px` was stale post-cleanup; `.search-bar .form-input border-right` was wrong era — current rule has no border-right). Plus three stale "Nav active state" subheaders in test-cases.md that the original V5 grep missed (it targeted "Candidates"/"Committees" specifically, not "Search"/"Races"). Pattern: cold-read audits catch drift that per-commit sweeps miss because the per-commit sweeps target the specific strings being changed; cold-reads target the whole surface for self-consistency.

**Stack tags:** IA reframing as a multi-arc piece of work (small code change forces large follow-on); border-architecture iteration as a "try, look, revert to baseline" exercise; latent-bug-surfaced-by-removed-cap pattern; .form-field-full as a flex-basis-modifier class (instead of :has() selector for predictability); search-typeahead vs filter-combo width/height scope distinction (banked refactor: split into .dropdown-search + .dropdown-input).

## How Sloane steered the work

**The IA reframing — "Candidates and Committees aren't destinations, they're search support"**

The session's foundational call. Without that framing, the changes would have read as polish: "let's tighten the spacing." With it, every follow-on decision had a coherent answer: max-width:800 because these aren't sprawling browse pages; no .page-header because they're not destination pages; aria-label-only because the search field doesn't need a label when it's the page; full-width input because the search is the surface, not a sub-component. The whole arc of 14 commits hangs from that single conceptual reframe.

**Option A vs B on the URL param flip — picking A despite the silent-flip risk**

When the terminated toggle inverted, I laid out three options for the `?terminated=1` URL param: A (reuse, flip semantic — silent flip for saved links), B (rename — graceful degradation), C (both — backward compat). I recommended B for cleanliness. You picked A. The reasoning held up: no committed code in this repo writes the old form, inbound bookmark traffic is approximately zero, and the cost of keeping two URL params for the rest of the project's life isn't worth the migration safety we don't need. The discipline: when graceful-degradation options exist but the underlying risk is approximately zero, take the simpler future.

**Pushing the visual iteration past "good enough" three times**

Three times this session you looked at shipped work and said "still not right" — once on the search input width (which surfaced the flex:1 latent bug), once on the chip treatment (which produced the surface2 background restyle), once on the border architecture (which surfaced the three-position iteration). Each pause produced a better answer. The discipline of "ship it, look at it, decide" repeated three times in one session is the same shape that previous T-arcs have surfaced — but the iteration cycle is shorter here (CSS-visual work iterates faster than data-layer work).

**The state-container border ask, then the revert**

You asked about adding the top-border to .state-loading to address the flicker. I confirmed it would work and surfaced that the same flicker exists on .state-no-results and .state-error transitions. You asked for all four (results-area + 3 state containers). I shipped it. Then you came back and said: revert the whole border architecture; put bottom borders back on .search-bar-wrap / .filter-bar-wrap; leave chips-wrap border-less. The revert wasn't a course correction on logic — it was a recognition that the original position was visually right and the migration had been solving the wrong problem. The chips-wrap staying border-less was the durable insight from the iteration.

**Recognizing the typeahead class confusion mid-arc**

When I extended the search-typeahead scope override for max-height, you noticed that `.typeahead-dropdown` is used for several conceptually different things (search, state-filter, click-combos) and asked whether a rename was worth pursuing now. We discussed Option A (just scope the change), Option B (modifier class), Option C (full rename .dropdown-search + .dropdown-input). You picked A for the immediate change and banked C as a future ticket. That's the right shape: the conceptual cleanup deserves a deliberate refactor with explicit decisions on child-class renames, not a side-effect of "while I'm in here." The banked ticket is documented in the design-system Typeahead notes so a future session has the context.

**The end-of-session cold-read ask itself**

"Do a cold read on all end-of-session rituals to address anything outstanding from this session with a high number of commits. And, beyond updates to documentation and tests, is there any cleanup that you'd recommend we address now?" The framing matters. The default close-out would have been: doc updates, append session log, push. The "anything outstanding" framing surfaces accumulated cruft that's easy to leave behind. The result: TESTING.md count fix, 3 test-log rows, design-system updates, CLAUDE.md drift fixes (2 items in the form-controls block), 3 cleanup items (.filter-bar max-width, .search-combo min-width, #state-empty wrapper). Worth banking: cold-read audits at session-close are dramatically higher-leverage on long sessions than short ones.

**Through-line:** the conceptual frame ("search-extension page") came from you and held through 14 commits of implementation rabbit-holing. The visual iteration ("try, look, decide") repeated three times and produced better answers each time, sometimes through reverts. The closing audit ask surfaced cruft that per-commit sweeps had missed.

## What to bring to Claude Chat

- **The `.typeahead-dropdown` triple-duty refactor.** Banked this session as Option C (`.dropdown-search` + `.dropdown-input` split). Migration cost: ~30 markup sites + 50 test references + 32 doc references + child-class decision. Deserves its own strategy doc + atomic execution. Worth a deliberate conversation about whether the rename is worth doing at all (the visual treatment is already shared; the class name is the only confusion) and what the child-class story looks like.
- **Search.html empty-state affordance.** Banked from the IA cleanup as Phase 2. Now even more "naked" after #state-empty wrapper retirement: search.html landing state is search bar + nothing below until the user types. Worth deciding: do we add a curated "Browse all candidates / committees" affordance? A "Recent searches" surface? A "Popular committees" cluster? Or leave it intentionally empty as the search-extension framing suggests? The pages are now demanding an answer.
...
Changelog:
- **9c — the breakdown-slot toggle (and the end of donut/Sankey coexistence).** Until now both the money-flow Sankey *and* the Raised/Spent donuts rendered on every detail view. 9c made the slot show exactly one: in-scope committees/candidates get the Sankey; gated ones (presidential, and committees with a non-federal "soft money" account) get the donut pair instead. The big call here was that the gated donut shows **no "not yet modeled" caption** — a complete, conserving donut isn't a degraded fallback, so it doesn't apologize for itself. A `breakdown_viz` field now rides the Page Viewed analytics event so we can see which picture each visitor actually got. The trickiest part was the tests: dozens of donut-rendering tests assumed the donut always rendered, so they had to move onto "gated" fixtures (a presidential candidate / a dual-account committee) to even see a donut — the default House/PCC mocks now show the Sankey.
- **9d — race page cold-load.** The original plan was candidate-row skeletons; you reversed it (a fixed-count skeleton lies about a list that might be 2 or 20 candidates), so the plain "Fetching race data…" loader stays. Instead I added a one-line comment explaining why race uses a shareable `?year=` URL (vs the profile pages' `#cycle` hash), and timed the two cold-load API calls against production: `/elections/` (the candidate list) is the slow, high-variance one (up to ~3s, no caching); the real fix is the already-banked server-side cache, not a spinner change.
- **9e — closed the flatten arc.** Verified the whole suite, signed off the live look, and swept the docs for stale references — including a cleanup of the manual test checklist, which still described clicking tabs that were removed a week ago.
- **Sankey spec banner refresh** so the data-model doc stops saying "coexisting with the donuts" (9c ended that).
- **Label consistency pass.** Sentence-cased the spend-donut categories + the Spending-by-Purpose bars to match the Sankey + raised donut. Renamed the committee spend wedge "Candidate Contributions" → **"Contributions to candidates"** — the field is money the committee *gave to* candidates, and the old name read backwards (as if from the candidate). Then aligned three more wedges that were the same fields under different names across the viz: **"Transfers in"**, **"Candidate self-funding"**, **"Offsets"** — with a single shared "Transfers in" tooltip so it means one thing everywhere.
- **Three field-coverage fixes, each from a "why is this in the Sankey but not the donut?" question:** (1) the candidate "Transfers in" wedge now coalesces all three transfer fields (the third is always zero on a candidate — added for cross-viz symmetry); (2) `fed_candidate_contribution_refunds` (a refund that comes *back* as a receipt) was a Sankey node but invisible on the donut — folded into "Offsets" on all three; (3) `coordinated_expenditures_by_party_committee` (a party committee's coordinated spending with its candidates — legally distinct, sometimes millions) was buried in the donut's "Other disbursements" remainder while the Sankey itemized it — now its own wedge beside its peers (IE, Contributions to candidates).

Field notes: The spine of the whole session was one idea — the breakdown slot's two views are *mutually exclusive but share labels*, so any divergence is a trap a real user can hit just by clicking from an in-scope entity to a gated one. "Offsets" meaning different fields in the Sankey vs the donut isn't pedantry; it's the same word pointing at different money on two screens of the same product. Once you see it that way, "label parity" stops being cosmetic and becomes a correctness property.

The investigations also kept surfacing a structural asymmetry worth remembering: the **spent** donut computes its catch-all as a remainder (`total − everything named`), so a field we forget to name is merely *invisible* — still counted. The **raised** donut has no remainder; it's a fixed allowlist, so a forgotten receipt field genuinely *vanishes* and the wedges quietly sum to less than 100%. That's why the offsets/refund omission on the raised side was a real under-count, while the coordinated-expenditures omission on the spent side was "only" a hidden category. Same symptom (a field in the Sankey but not the donut), two different severities — and the fix differs accordingly.

Two decisions deferred to you that are worth flagging forward: the `--cat` color ramp's seven hues are now fully assigned, so coordinated expenditures had to take a neutral grey rather than a first-class hue — the next new category forces a real palette call. And a recurring rhythm this session: every label/field change here is also a Sankey-spec change, because the two viz are now genuinely one system — the doc churn (sankey-examples.html, sankey-data-model.md, chart-color-palette.html) is the cost of that coupling, and it's worth it.

Stack tags: (none new — ECharts 5.5.1, Chart.js 4.4.0, Playwright, Cloudflare Pages all already in the stack)

## How Sloane steered the work

**"Investigate only" — diagnose before you fix.** Almost every change after 9c started as a question you scoped explicitly as investigation, not a fix request ("why is X labeled differently / missing / in the Sankey but not the donut?"). That discipline is the reason the fixes were surgical: each write-up separated a *real* defect (the JFC transfer mislabeled "authorized committees"; coordinated expenditures buried in "Other") from a *harmless* asymmetry (the party-transfer field that's structurally always zero on a candidate). Without the investigate-first rule, I'd have "fixed" the harmless ones the same way and added dead code.

**Reversing your own locked decision on the race skeleton.** Decision 4 was locked to candidate-row skeletons. You overrode it when the reasoning didn't survive contact — a fixed-N skeleton implies a known row count, but a race might have 2 candidates or 20, so it would actively mislead. That turned 9d from a UI build into an honest "keep the plain loader, go find the real slowness" — and the diagnosis pointed at the API/caching layer, where the fix actually lives. Killing a locked plan because its premise broke is harder than it sounds.

**"Why wouldn't we do the same for the Sankey?"** My first proposal folded the contribution-refund into the donut's "Offsets" but left the Sankey's separate node. You caught instantly that this would *re-introduce* the exact divergence the whole pass was eliminating — "Offsets" would mean different fields on the two screens. That one question upgraded the fix from "patch the donut" to "make the field-set identical in all three viz." You were holding the invariant more firmly than I was.

**"Why not --cat-4?"** When I claimed there was no safe hue to share, you pushed on the obvious candidate (`--cat-4`, Contributions to candidates). It forced a precise answer — that share works on the *raised* donut only because self-funding and non-fed transfers are mutually exclusive *by committee type*, whereas coordinated expenditures and contributions-to-candidates *coexist* on the same party committee, so they'd collide. You didn't accept "trust me, it's full"; you made me show the mechanism, and then made the call (the grey + a deliberate ordering) rather than defaulting to a heavier new-token change.

**Settling the tooltip before the relabel.** On the "Transfers in" rename you asked to lock the *tooltip wording* first, separately, before touching code — quoting the current copy and choosing the shared version. Sequencing the copy decision ahead of the mechanical change kept the two from getting tangled, and meant the relabel landed in one clean pass.

**Folding vs itemizing — the same question, two different answers, and you saw why.** The refund got *folded* into Offsets; coordinated expenditures got *itemized* as its own wedge. Those look contradictory until you hold the distinction you were tracking: the refund is a tiny contra-receipt that belongs *with* offsets (same family); coordinated expenditures is a legally-distinct, sometimes-multimillion party activity that belongs *beside* its peers. You treated "fold or itemize?" as a judgment about what the category *is*, not a mechanical default.

**The through-line:** you ran the session as a guardian of one promise — *the same money, described the same way, wherever the user sees it* — and tested every micro-decision (casing, wording, which fields, which color, fold-or-itemize) against it. The counter-discipline was just as consistent: where a lighter answer kept the promise, you took it (plain loader over skeleton, grey-reuse over a new token), refusing to spend complexity you didn't need.
