...
- **Top Committee Contributors** — PACs, party committees, and other committee-type contributors giving their own money. Legally distinct from conduit flow. Live on committee.html via live paginated Schedule A API with a 100-page threshold gate and an honest `"Unable to show due to high transaction volume."` empty state for mega-committees. The Session 3 bundle (2026-04-20) added a KV-backed pre-computation path (`top_committees:{cmte_id}:{cycle}` from pas2 via DuckDB) but disabled it the same day after discovering that pas2's `NAME` field stores recipient names, not giver names — producing visually wrong results. The feature flag (`ENABLE_TOP_COMMITTEES_PASS`) is retained for a future session that wires in FEC's Committee Master File (`cm.txt`) as the authoritative committee_id → registered_name source; see `strategy/cm-txt-integration.md`. candidate.html continues to use the live paginated API (candidate committees never hit the mega threshold).
