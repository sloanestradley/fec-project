...
- URL anchor includes cycle: e.g. `candidate#2026` (bare cycle — the `#summary`/tab segment was retired in T-remove-profile-tabs; see the Financials note below)
...
- Cycle is also encoded in the URL anchor (e.g. `#2024` — bare cycle, no tab segment post T-remove-profile-tabs)
...
*Raised*
- Breakdown by contributor type (itemized individuals, unitemized individuals, PACs & other committees, party committees, candidate authorized committees, candidate self-funding, loans, federal funds, offsets, other receipts) — donut chart. **Superseded:** the Raised + Spent donuts were replaced by a single **Money flow (Sankey)** diagram that shows Raised→Committee→Spent in one conserving picture (see Definitions; Sankey build 2026-06-09, scope toggle shipped 2026-06-10). The slot is now **mutually exclusive on the gate** (T-profile-flatten 9c): in-scope entities render the Sankey; the still-gated presidential case renders the donut pair as a complete first-class view (no longer a "fallback chart" — see Definitions); dual-account committees were **un-gated in Step 5 (2026-06-11)** and now render the Sankey directly. The donuts no longer coexist with the Sankey.
- Geography of individual contributions displayed in a US heat map visualization
- **Top Individual Contributors** — ranked list of the largest individual donors to the committee (by cycle), surfaced as an entity-level view to complement the aggregate geography map. Live on **committee.html only** as of Session 3 (2026-04-19, candidate.html surface built and then rolled back 2026-04-20). Served from pre-computed bulk data (FEC indiv files → daily DuckDB GROUP BY in GitHub Actions → Cloudflare KV namespace `fecledger-aggregations`, key `top_contributors:{cmte_id}:{cycle}`). Pages Function at `functions/api/aggregations/[[path]].js` reads KV and returns top 25 per committee; UI shows top 10. Accuracy 100% — DuckDB does external spill-to-disk GROUP BY, no pruning. **Cycle coverage:** every even-year cycle from 1980 to 2026 after the 2026-04-21 historical backfill. **Scope:** only committees that appear as a recipient in pas2 OR have ≥500 post-memo individual rows in indiv get pre-computed entries. Committees outside that scope fall through to a paginated API fetch (gated at 100 pages / 10,000 transactions) or render `"Data not available for this committee."` when even that would be incomplete. **Deliberately not on candidate.html:** individual contributions to a single candidate committee are legally capped at ~$3,300 per election, so a top-10 list on a candidate page is dominated by max-out donors and carries little differentiating signal between candidates. The surface is only meaningful for committees (PACs, parties, leadership PACs, super PACs, conduits) where contribution limits vary.
- **Top Committee Contributors** — PACs, party committees, and other committee-type contributors giving their own money. Legally distinct from conduit flow. Live on committee.html via the same KV-first branch tree as Top Individual Contributors: hit → use bulk data, miss + pages ≤ 100 → live paginated Schedule A API, miss + pages > threshold → `"Unable to show due to high transaction volume."` empty state. KV key `top_committees:{cmte_id}:{cycle}` aggregated from pas2 via DuckDB (Session 3 bundle, 2026-04-20), with the committee-name source resolved in Session 4B (2026-04-21) by ingesting FEC's **Committee Master File (`cm.txt`)** and LEFT-JOINing it in `buildCommitteesAggSql()` — cm.txt's `CMTE_NM` is the authoritative registered name for each `CMTE_ID`, vs. pas2's `NAME` which is the free-form `recipient_name` the filer wrote (often a DBA or affiliate-branded string). **Cycle coverage:** every even-year cycle from 1980 to 2026 after the 2026-04-21 historical backfill. ActBlue/WinRed and the national party committees (DNC/RNC/DSCC/NRSC/DCCC/NRCC) still fall through to the empty state — pas2 doesn't capture their inbound flows (memo rows and "transfer" transaction types live elsewhere), independent of the name-source fix. candidate.html's Top Committee Contributors now routes through the same `top_committees` KV path (T-candidate-raised-fetch-architecture, 2026-06-05): the candidate `is_individual=false` crawl is conduit-memo-driven and routinely exceeds the 100-page cap (Marie's PCC ~198 pages), so a page-1 probe per sub-cycle decides between the merged per-sub-cycle KV slices and a "high volume" bail — though it never approaches the 500k mega threshold (see the Mega-committee definition).
...
*Spent*
- Breakdown by spend category — donut chart. **Superseded-in-progress** alongside the Raised donut by the **Money flow (Sankey)** (see the Raised note above + Definitions); the donut remains the fallback for gated cases.
...
- **Spend timeline on the Spent section (candidate + committee):** A spend-over-time line chart parallel to the Raised section's chart. Deferred from Phase 1 — the category/purpose/vendor breakdown is sufficient for current use. Add when the Raised chart pattern is ready to be reused across both pages.
...
- Local dev: `npm run dev` from project root → `http://127.0.0.1:8788/` (runs `wrangler pages dev` over the staged `dist/`, so Pages Functions — the `/api/fec/*` proxy and the `/candidate/:id` / `/committee/:id` clean-URL routers — and `_redirects` all work). Requires a gitignored `.dev.vars` with `API_KEY=…`. Both URL forms work, e.g. `127.0.0.1:8788/candidate/H2WA03217` (clean) or `?id=H2WA03217`. **`python3 -m http.server` is NOT sufficient** — it doesn't run Pages Functions (the API proxy 404s) or honor `_redirects`.
...
- **Independent expenditures (Schedule E)** — outside spending for or against a candidate, surfaced in two places: (1) on candidate pages (Summary section), showing all committees making IEs targeting that candidate, with support/oppose breakdown; (2) on committee pages (Spent section), as a drill-down behind the existing `independent_expenditures` line item in the spending donut — showing which candidates the committee is targeting and in which direction. Particularly meaningful for leadership PAC and super PAC profiles where IEs are the primary political activity. Endpoint: `/schedules/schedule_e/`; `support_oppose_indicator: 'S'` = supporting, `'O'` = opposing. Field shapes unverified — verify against live response before building.
- **Refund spike detection** — flag unusual outflows in `contribution_refunds` on the candidate/committee Raised section as a potential signal of donor retreat or compliance issues. Threshold for "spike" requires validation with John before implementation.
- **Overhead ratio (committee pages)** — derived metric on the committee Spent section: `operating_expenditures / total_disbursements`. High ratios may indicate a committee oriented around consultant fees rather than political impact. Threshold requires validation — do not hardcode without John's input.
- **Dark money signals** — flag Schedule A receipts where `entity_type: 'ORG'` and contributor name contains LLC or INC as potential non-disclosing entities. Surfaces in two places: (1) on candidate pages (Raised section), flagging dark money contributions to the candidate's principal committee; (2) on super PAC committee pages, flagging dark money flowing into the PAC itself. Heuristic approach — validate signal reliability and non-partisan framing with John before building.
...
- ~~**Server-side API proxy**~~ — ✅ **Done (2026-04-14).** Migrated from Netlify to Cloudflare Pages. FEC API calls now route through `functions/api/fec/[[path]].js`; API key stored as a Cloudflare secret, no longer client-visible. Remaining work: server-side *caching* for races.html enrichment calls (Cloudflare KV). The mega-committee Schedule A aggregation problem was partially addressed by Session 3 (2026-04-19/20) via the pre-computed bulk pipeline (pas2 + indiv → DuckDB → KV) — Top Individual Contributors and Top Committee Contributors are both live on committee.html, the latter via the cm.txt Committee Master File integration (Session 4B, 2026-04-21; `strategy/cm-txt-integration.md`, executed).
...
- ~~**Pages project is Direct Upload, not git-connected**~~ — ✅ **Done (2026-04-21).** Migrated to a new git-connected Cloudflare Pages project at `fecledgerapp.pages.dev`; push-to-main now triggers an automatic deploy via `scripts/pages-build.sh` (delegates to `scripts/stage-site.sh`, the single allowlist for the deploy surface — fails closed, so a new sensitive file at repo root won't accidentally ship). Push-to-deploy chain provable via the `data-deployed-via="git"` attribute on the deployed root page. Old `fecledger` Direct Upload project deleted in the Cloudflare dashboard; pre-delete safeguard rule (visit the URL before deletion to confirm it isn't currently-live) lives in `CLAUDE.md` for any future Pages project retirement. Originally surfaced 2026-04-17 when a binding change sat invisible for ~3 weeks because git pushes never triggered deploys. **Forward-looking lessons:** (1) `wrangler pages deploy` CLI produces Direct Upload projects with no git linkage and no in-place upgrade path — for any future Pages project, prefer the Dashboard → Pages → Connect to Git OAuth flow. (2) The general Cloudflare "Create application" button now lands in a Workers-with-static-assets flow that does NOT auto-discover `functions/`; Pages creation lives at the explicit URL `/<account>/pages/new/provider/github`. Full execution detail in `claude-to-claude.md` (Phase 1 + Phase 2 entries) and `test-cases.md` rows.

---

## Open items (not prioritized)

- ~~**Tooltip UI debt**~~ — ✅ **Resolved (2026-05-29 → 2026-06-01).** The native `title`-attribute pattern was replaced project-wide. (1) Party tags dropped `title=` entirely — the visible label now carries the full party info (T-party-helpers dual-field rewrite), so no hover popup is needed. (2) A designed JS tooltip component shipped (`initTooltips` — info-icon trigger + body-portaled popup with full touch / keyboard / a11y support) and was applied to all V1 methodology surfaces: Raised-donut wedges, choropleth, Spending by Purpose, cycle-index archive divider, and the Raised:Spent ratio stat label. The original note envisioned a CSS-only `data-tooltip` pattern; the project went with the richer JS component instead (see CLAUDE.md `initTooltips` entry; `strategy/data-notes-mapping-v2.md` for the per-surface mapping).
...
- **Primary losers in active cycle candidate counts** — The FEC API does not filter out candidates who lost a primary from the /elections/ endpoint results. Challenger counts and race pages will overcount active candidates mid-cycle (between primary and general). Fixing this requires cross-referencing election results data, which the FEC API does not provide — likely requires a third-party data source. Noted as a known gap; acceptable for now. **Update (2026-07-17):** race.html + candidate.html now show the *accurate* total (`pagination.count`) rather than a per_page-capped undercount — which makes this overcount **more visible**, not less (e.g. "President 2024: 869 candidates" surfaces every write-in / $0 / ballot-access filer). This was a deliberate call — the count is raw FEC (consistent with the un-deduped race `total`); a "funded candidates" count (receipts > 0) that would sidestep the overcount was considered and **declined** this session as a separate, definition-fuzzy feature. So the gap stands, now with an accurate raw number in front of it.
...
- **Race page — defining "unserious" candidates:** Candidates with low or no financial data should be shown with reduced visual hierarchy on race pages, but what threshold defines "unserious"? A declared candidate with zero filings is different from one with minimal activity. **Partial progress (T-race-page-UI, 2026-06-03):** the **zero-filing extreme** is now handled — a candidate whose `/elections/` totals are `$0/$0` (the live API returns `0.0`, not null, for non-filers) renders "No financial activity reported" in place of the stat row instead of a `$0/$0/$0` roll-call. This addresses the "zero filings" end of the spectrum but **not** the "minimal activity" threshold (e.g. a candidate with $5K raised) — reduced visual hierarchy for *low-but-nonzero* candidates is still an open design question. **Ordering (2026-06-12):** the race candidate list now sorts by `total_receipts` descending (non-filers sink to the bottom), so the reduced-hierarchy question is now purely about *treatment* (how a low/no-activity card should look), not *placement* — funded candidates already lead. *(Validate with John — what would a strategist actually want to see, and where's the "serious" cutoff?)*
...
- **Raised section — "Transfers In" category:** John flagged confusion between "Transfers In" and "PAC / Other Committees" — they appear to overlap because JFA contributions are categorized as "Transfers In" by the FEC rather than under "Individual" or "PAC." Total raised figures match campaign-confirmed numbers so double-counting is not occurring. Two actions: (1) confirm `receipt_type` values behind "Transfers In" via Schedule A *(still open)*; (2) ✅ done — the committee Raised donut's "Transfers in" wedge carries a plain-language tooltip (the candidate donut's "Transfers in" wedge likewise — the two were aligned to the same label). *(FEC note: contributions received as part of a joint fundraising transfer are included in "Transfers In" rather than "Individual contributions" in some data files)*
...
- **Special election surfacing on candidate cycle index:** When a cycle in a candidate's history was a special election — detectable because the candidate entity endpoint's `election_years` array surfaces it as a distinct entry (the `/candidate/{id}/history/` call that previously sourced this was retired on candidate.html in T-history-retire; the entity call carries the same data) — should the cycle row carry a visual indicator (e.g. an "SP" tag or "Special election" label)? The Gillibrand case confirms the data is there and visible; the question is whether the distinction is signal or noise for a strategist audience. *(Validate with John before building)*
- **Race page — Senate seat disambiguation in multi-race cycles:** When a state holds two Senate elections in the same cycle (one regular + one special), race.html must present them as distinct contests. Currently FECLedger has no reliable term class signal to label them ("WA Senate, Class 1" vs. "WA Senate, Class 2"). Verify whether a `term_class` field exists on the OpenFEC `/elections/` or `/elections/search/` response — not confirmed in live data. UX question: how do we label these contests clearly for users who don't know Senate class structure? *(Validate with John)*
...
- **Super PAC (independent-expenditure-only committee):** Raises and spends unlimited sums on independent expenditures supporting or opposing candidates, but makes **no** contributions directly to candidates.
- **Hybrid PAC (Carey committee):** Maintains two separate accounts — a traditional (hard-money) account that contributes to candidates under normal limits, and a non-contribution account that raises/spends unlimited sums on independent expenditures, with a wall between them. **Note:** both of a hybrid PAC's accounts are *federal* — distinct from a non-federal (state/local soft-money) account — the thing that defines a "dual-account" committee, which the Money-flow Sankey now MODELS directly (FEA + non-federal transfers); it was a scope *gate* until the Step-5 un-gate (2026-06-11).
...
- **Money flow (Sankey):** A Raised→Committee→Spent flow diagram on the candidate/committee detail view, built to replace the Raised + Spent donuts with a single conserving picture of where a cycle's money came from and went (Sankey build, 2026-06-09; Apache ECharts, self-hosted). It rests on the **FEC conservation model**: a `/totals/` record's receipt fields and disbursement fields each form an exact MECE partition that sums to the penny — *provided* you avoid three trap-field classes (subtotals like `contributions`, net figures like `net_operating_expenditures`, and duplicate aliases like `fed_receipts`≡`receipts` on fed-only committees). Cash-on-hand at cycle start is **derived** (`cohEnd + disbursements − receipts`), not the filed opening balance, so the two sides always balance. **Scope gate (after the Step-5 Gate-1 un-gate, 2026-06-11):** only **presidential committees** are gated (Form 3P has public-financing/exempt leaves the model doesn't fully render yet, and Form-3P loans land in a raised-side field the Loans node doesn't read). **Dual-account committees were UN-GATED** — the Sankey now models them directly (a Federal Election Activity node + non-federal transfers commingled into "Transfers in" + remainder catch-alls, conserving to the penny). For the still-gated presidential case the breakdown slot renders the **Raised + Spent donut pair** instead of the Sankey; after the dual-account donut fix the donuts are complete + conserving, so that gated view is a **first-class breakdown, not a degraded fallback** — it carries **no "not yet modeled" caption** (T-profile-flatten 9c). The scope toggle shipped 2026-06-10: the slot is **mutually exclusive per entity** (Sankey XOR donut pair), so the Sankey no longer coexists with the donuts. The gate *detector* (`sankeyGateReason`) now only selects which viz the slot mounts.
...
- **Mega-committee:** Informal term for committees whose Schedule A volume exceeds what client-side pagination can handle — measured threshold ~100 pages at per_page=100. Examples: ActBlue (measured 11,163,722 rows / 111,638 pages in 2024), WinRed, DNC/RNC/NRSC/DSCC/DCCC/NRCC. For these committees, FECLedger's Top Committee Contributors and Top Conduit Sources tables show an "Unable to show top committees due to high transaction volume" empty state on committee.html until the server-side aggregation architecture lands (see `strategy/hosting-migration.md`). Candidate.html DOES hit this limit (corrected 2026-06-05, T-candidate-raised-fetch-architecture): the candidate crawl is conduit-memo-driven, so `is_individual=false` routinely exceeds the 100-page cap even for normal House candidates (Marie's PCC ~198 pages), though it never approaches the 500k mega threshold. Candidate now bounds it with the same page-1 probe + `top_committees` KV path as committee.html (Committees from merged per-sub-cycle KV on bail, Conduits → "high volume").
- **Senate term class:** The U.S. Constitution divides all 100 Senate seats into three classes (Class 1, 2, and 3), staggered so roughly one-third of the Senate faces election every two years. Each state always has its two Senators in different classes — so two Senate races in the same state never occur in the same cycle under normal circumstances. Class assignment is permanent: a Senator elected to a Class 2 seat retains that class for their full tenure. In special elections, the winner inherits the class of the vacated seat and serves the remainder of the unexpired term. Relevant to FECLedger for race page disambiguation — when a state holds both a regular and a special Senate election in the same cycle, they must be presented as distinct contests with distinct class labels.
- **Special election (Senate):** A Senate election held outside the regular election cycle to fill a vacancy caused by death, resignation, or expulsion. FEC tracks special elections via distinct `election_type_id` values on the `/election-dates/` endpoint: `SP` (special primary), `SG` (special general), `SGR` (special general runoff). Financial totals for a special election period are folded into the candidate's standard FEC reporting periods — `/candidate/{id}/totals/` does not distinguish special from regular — but `/candidate/{id}/history/` surfaces special elections as distinct entries in the `election_years` array. Example: Kirsten Gillibrand (S0NY00410) shows both 2010 (special general, appointed 2009 to fill Hillary Clinton's seat) and 2012 (her first regular election) as separate rows in her cycle index on FECLedger. This makes the candidate index view the only place in the current product where special election history is directly visible.
