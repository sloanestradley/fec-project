...
- ~~**Server-side API proxy**~~ — ✅ **Done (2026-04-14).** Migrated from Netlify to Cloudflare Pages. FEC API calls now route through `functions/api/fec/[[path]].js`; API key stored as a Cloudflare secret, no longer client-visible. Remaining work: server-side *caching* for races.html enrichment calls (Cloudflare KV). The mega-committee Schedule A aggregation problem was partially addressed by Session 3 (2026-04-19/20) via the pre-computed bulk pipeline (pas2 + indiv → DuckDB → KV) — Top Individual Contributors is live on committee.html; Top Committee Contributors is scoped in `strategy/cm-txt-integration.md` pending cm.txt work.
...
- **Special election surfacing on candidate cycle index:** When a cycle in a candidate's history was a special election — detectable because `/candidate/{id}/history/` surfaces it as a distinct `election_years` entry — should the cycle row carry a visual indicator (e.g. an "SP" tag or "Special election" label)? The Gillibrand case confirms the data is there and visible; the question is whether the distinction is signal or noise for a strategist audience. *(Validate with John before building)*
