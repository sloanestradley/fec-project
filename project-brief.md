...
- **Raised section — "Transfers In" category:** John flagged confusion between "Transfers In" and "PAC / Other Committees" — they appear to overlap because JFA contributions are categorized as "Transfers In" by the FEC rather than under "Individual" or "PAC." Total raised figures match campaign-confirmed numbers so double-counting is not occurring. Two actions: (1) confirm `receipt_type` values behind "Transfers In" via Schedule A *(still open)*; (2) ✅ done — the committee Raised donut's "Transfers in" wedge carries a plain-language tooltip (the candidate donut's "Candidate authorized committees" wedge likewise). *(FEC note: contributions received as part of a joint fundraising transfer are included in "Transfers In" rather than "Individual contributions" in some data files)*
