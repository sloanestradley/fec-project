...
- **Top Individual Contributors** — ranked list of the largest individual donors to the committee (by cycle), surfaced as an entity-level view to complement the aggregate geography map. Currently live on committee.html via client-side Schedule A aggregation; being migrated to pre-computed bulk data in Session 3 (FEC indiv files → daily DuckDB GROUP BY in GitHub Actions → Cloudflare KV namespace `fecledger-aggregations`, key `top_contributors:{cmte_id}:{cycle}`). A Pages Function reads KV and returns top 25 per committee; UI shows top 10. Accuracy 100% — DuckDB does external spill-to-disk GROUP BY, no pruning. **Scope:** only committees that appear as a recipient in pas2 OR have ≥500 post-memo individual rows in indiv get pre-computed entries; committees outside that scope fall back to client-side aggregation (existing path). Surface to be added to candidate.html for parity in Session 3.
