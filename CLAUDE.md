...
### What's working
- Profile header: `.candidate-race-label` div above `.candidate-row` renders a long-form race label (`formatRaceLabelLong()`) in red-700 Oswald 400 uppercase, linking to the race page; `.candidate-row` below has candidate name + party tag (race tag removed from redesign branch) + "Committees (N) →" trigger floating right via `margin-left:auto`
