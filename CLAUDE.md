...
**Party tag render order:** Race tag first, party tag second — on candidates.html and search.html. This is the canonical order for browse/search results. On candidate.html as of T15 (2026-05-11), the race element is the first child of `.meta-row` (a `<span class="candidate-race-label">` containing the race link), followed by the party tag — same canonical order as the browse/search surfaces, just realized via a `.candidate-race-label` styled at heading-style (Oswald 400 1.25rem uppercase red-700) instead of a `.tag-neutral` tag. Visual mismatch against the 0.625rem mono tags is acknowledged; a meta-row context variant is pending explicit direction. Party tag always includes `title="..."` via `partyTooltip(c.party, c.party_full)` — native browser tooltip on desktop hover.
