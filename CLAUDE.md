...
- **Top Committee Contributors on committee.html — mega-committee gap (2026-04-14):** The committee Raised tab paginates `/schedules/schedule_a/?is_individual=false` to exhaustion for accurate totals, but adaptively gates at 100 pages (10,000 transactions) — any committee above that threshold shows an empty-state message ("Unable to show top committees due to high transaction volume.") rather than a silently-capped approximation. This affects mega-conduits and national party committees: ActBlue (measured 11,163,722 rows / 111,638 pages in 2024), WinRed, DNC, RNC, NRSC/NRCC/DSCC/DCCC-class committees. For these, client-side aggregation is infeasible — 111k+ requests per page load would exhaust the shared 7,200/hour key in a single visit and take hours. The permanent fix is a Netlify Function that walks Schedule A server-side on a schedule, caches the aggregated top-N by contributor in Blob storage, and serves the cached result to clients in one round-trip. This is a separate forcing function for the Netlify proxy work from the races.html note above — races.html needs proxying for request-volume/rate-limit reasons; committee.html Top Committee Contributors needs proxying for *correctness* on the mega-committees where client-side pagination is architecturally impossible. Same fix, distinct drivers. candidate.html does not hit this limit because candidate committees have bounded incoming committee-to-committee transfer volume.
