...
scripts/
  ...
  precompute-aggregations.js    — DuckDB-based pre-computation of top contributors AND top committee contributors per committee per cycle.
                                  Runs after ingest-bulk.js completes. For cycles 2024 + 2026: downloads pas2 + indiv CSVs from R2 to /tmp,
                                  runs two SQL GROUP BY queries (spill-to-disk bounded memory, 100% accurate totals), writes top 25 per
                                  in-scope committee to Cloudflare KV via REST bulk PUT. Wipes the namespace entirely before writing to
...
- ~~Session 3~~ ✅ Wired pre-computed aggregations to **both** Top Individual Contributors and Top Committee Contributors surfaces. Pages Function at `functions/api/aggregations/[[path]].js` exposes two routes (`top-contributors` and `top-committees`), both returning `{results, source}` where `source='bulk'` on KV hit and `source='api'` on miss (client branches on body). **KV-first branch tree** applied to both surfaces on committee.html: hit → use bulk data, miss + pages ≤ PAGE_THRESHOLD (100) → paginate live API to exhaustion for accurate totals, miss + pages > threshold → honest empty state (`"Data not available for this committee."`). `cycleOrAll === 'all'` for Top Individual Contributors uses the same KV-first tree scoped to `ALL_CYCLES[0]` with a data-note disclaimer clarifying the scoping; Top Committee Contributors is hidden entirely on All time (aggregation across cycles isn't meaningful). **Top Conduit Sources** stays live-API-only — no bulk-data equivalent — and becomes unavailable on mega-committees even when KV serves top committees from bulk data. **candidate.html** got a new Top Individual Contributors card below `#conduits-card`, scoped to the *latest sub-cycle* only (`subCycles[subCycles.length-1]`) — keeps KV data 100% accurate per its key shape; Senate/Presidential candidates show just the most recent 2-year period with a disclaimer in the data note. candidate.html's Top Committee Contributors continues to use the live paginated API since candidate committees never hit the mega threshold. KV row shape: `{name, entity_type, city, state, total}` for individuals (mode() on city/state for donors with mixed location history); `{name, entity_type, committee_id, total}` for committees (committee_id future-proofs linking).
...
- ~~**Top Committee Contributors on committee.html — mega-committee gap**~~ ✅ **Closed 2026-04-20.** Session 3 bundle added `top_committees:{cmte_id}:{cycle}` KV keys (aggregated from pas2 via DuckDB), a `top-committees` route on the Pages Function, and mirrored the KV-first branch tree on committee.html's Top Committee Contributors block. Mega-committees (ActBlue, WinRed, DNC, RNC, NRSC/NRCC/DSCC/DCCC) now serve accurate top-25 committee contributors from bulk data instead of the empty state. **Top Conduit Sources stays live-API-only** — no bulk-data equivalent for memo rows — so mega-committees still show `"Unable to show conduit sources due to high transaction volume."` for conduits even when top committees render from KV. This is honest: the committees table is decoupled from the conduits table, and the two rendering states can differ per committee. candidate.html's Top Committee Contributors continues to use the live paginated API (candidate committees never hit the mega threshold, so the KV path would add complexity without user-visible benefit).
