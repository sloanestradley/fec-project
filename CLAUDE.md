...
- **Testing:** Playwright (`@playwright/test`) — `npx playwright test` runs 416 structural tests (mocked API); `npm run test:smoke` runs 5 live-API smoke tests against `https://fecledger.pages.dev`. See `TESTING.md`.
...
scripts/
  ...
  precompute-aggregations.js    — DuckDB-based pre-computation of top contributors per committee per cycle. Runs after
                                  ingest-bulk.js completes. For cycles 2024 + 2026: downloads pas2 + indiv CSVs from R2 to /tmp, runs
                                  SQL GROUP BY queries (spill-to-disk bounded memory, 100% accurate totals), writes top 25 per in-scope
                                  committee to Cloudflare KV via REST bulk PUT. Wipes the namespace entirely before writing to prevent
                                  stale/inaccurate entries from coexisting with fresh data. TTL: 7 days. Two key patterns are defined;
                                  only the first is currently populated (see ENABLE_TOP_COMMITTEES_PASS feature flag in the file):
                                    ...
                                    • `top_committees:{cmte_id}:{cycle}` — temporarily DISABLED (flag=false, 2026-04-20). Would aggregate
                                      pas2 WHERE OTHER_ID=this committee grouped by CMTE_ID (giver) and write rows of shape
                                      {name, entity_type, committee_id, total}. First ship stored pas2 NAME (which is recipient_name, not
                                      giver_name) as the display name and produced visually wrong data; flag off until cm.txt integration
                                      provides a reliable committee_id → registered_name lookup. See strategy/cm-txt-integration.md.
...
- **Top Committee Contributors on committee.html — KV path partially shipped, temporarily disabled (2026-04-20):** The Session 3 bundle (2026-04-20) added the full infrastructure for KV-backed Top Committee Contributors — `top_committees:{cmte_id}:{cycle}` KV keys (aggregated from pas2 via DuckDB), a `top-committees` route on the Pages Function, and a KV-first branch tree in committee.html. Running the pipeline revealed a data-semantics bug: the pas2 `NAME` column stores the **recipient's** name (Schedule B `recipient_name`, often a DBA or affiliate-branded name), not the **giver's** name. The initial KV entries displayed recipient-named strings as if they were giver names — producing rows like Marie for Congress showing as its own top contributor because JFAs filed with recipient_name="MARIE FOR CONGRESS". Mitigation: `ENABLE_TOP_COMMITTEES_PASS = false` at the top of `scripts/precompute-aggregations.js` gates the second SQL pass, so pipeline runs no longer write `top_committees:*` keys. The frontend branch tree, Pages Function route, and `buildCommitteesAggSql()` SQL builder are all intact — re-enabling is a flag flip once the name source is correct. **The scoped fix is documented in `strategy/cm-txt-integration.md`:** ingest FEC's Committee Master File (`cm.txt`) into the pipeline, use it as the authoritative `committee_id → registered_name` source in the pas2 aggregation, then flip the flag. Half a session of work. Also noted there: the conduit mega-committees (ActBlue, WinRed — dominated by memo rows not in pas2) and national party committees (DNC, RNC, DSCC, DCCC — inbound dominated by FEC "transfers" not "contributions", also not in pas2) are **separate gaps** that cm.txt alone doesn't close. candidate.html's Top Committee Contributors continues to use the live paginated API unchanged.
