...
**Race context line:** `.race-context-line` (in `styles.css`) is inline serif prose (IBM Plex Serif 0.875rem) for contextual sentence-format content. No background, no padding, no border, no uppercase. Despite the prior `.tag-context` name (renamed 2026-05-13), this is NOT a `.tag` variant — it shares zero declarations with `.tag`. `display:flex; width:fit-content; max-width:100%; flex-wrap:wrap` — hugs content on wide viewports; constrained by `max-width:100%` on narrow viewports, at which point `flex-wrap` kicks in and the link right-aligns via `margin-left:auto`. `.race-context-line-text` has `flex:1; min-width:0` so it fills the row when wrapping occurs. `.race-context-line a` has `margin-left:auto; white-space:nowrap`. Used for the race context sentence on the candidate profile (inside `#race-context-bar`). Promote from `candidate-only` to `stable` in `design-system.html` when used on a second page.
...
- Race context sentence (`.race-context-line` sourced from `/elections/`, skeleton while loading) lives in `#race-context-bar`, the bottom-most child of `#summary-strip` (T21 follow-up, 2026-05-07) — visible on all tabs as part of the header zone above the tabs bar
