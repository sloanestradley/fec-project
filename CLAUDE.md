...
**Tab-skeleton variants (T12):** `.tab-skeleton` is the block-level variant of `.skeleton` for chart/table cells inside Raised + Spent tabs on candidate.html and committee.html. Four sizes (defined in `styles.css`): `.skeleton-donut` (280px), `.skeleton-map` (360px), `.skeleton-table` (240px), `.skeleton-bars` (240px). Used with the **title-always-visible pattern**: the section title (`.raised-cell-title`, `.donors-head`) renders immediately and stays visible during the loading window; only the chart canvas / table body area swaps between skeleton and content via `display:none` / `display:block` toggling on per-section `*-skeleton` and `*-content` IDs. **`.tab-still-loading`** is the 10s "still working" augmentation under slow-tier skeletons (Mono 10px italic, muted, centered) — fired by a `setTimeout` in `kickoffRaisedSlowFetch` / `kickoffSpentFetch` when the fetch is still pending at 10s. Cleared on resolve, error, or cycle change.
