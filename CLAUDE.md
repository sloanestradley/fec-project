...
- Session 3 — Wire pre-computed aggregations to the Top Individual Contributors surface. committee.html **already has** this surface (see `#individual-donors-tbody` / `renderIndivDonors()` in committee.html), currently fed by client-side Schedule A aggregation (`/schedules/schedule_a/?is_individual=true` paginated and summed in JS). Session 3 replaces that client-side path with a Pages Function (e.g. `functions/api/aggregations/[[path]].js`) that reads `env.AGGREGATIONS.get('top_contributors:{cmte_id}:{cycle}')` and serves the pre-computed JSON — faster, memo-clean, and scales to mega-committees where client-side aggregation is infeasible. Session 3 also adds the same surface to candidate.html (it does not have a Top Individual Contributors card today). **This does NOT resolve the existing Top *Committee* Contributors mega-committee empty state** on committee.html — that surface uses Schedule A `is_individual=false` (committee-to-committee money, pas2 bulk data territory), which is a separate aggregation problem. See architectural debt below for the analogous path to close that gap.
...
- **Top Committee Contributors on committee.html — mega-committee gap (2026-04-14):** The committee Raised tab paginates `/schedules/schedule_a/?is_individual=false` to exhaustion for accurate totals, but adaptively gates at 100 pages (10,000 transactions) — any committee above that threshold shows an empty-state message ("Unable to show top committees due to high transaction volume.") rather than a silently-capped approximation. This affects mega-conduits and national party committees: ActBlue (measured 11,163,722 rows / 111,638 pages in 2024), WinRed, DNC, RNC, NRSC/NRCC/DSCC/DCCC-class committees. For these, client-side aggregation is infeasible — 111k+ requests per page load would exhaust the shared 7,200/hour key in a single visit and take hours. The permanent fix is a Cloudflare Worker (scheduled cron) that walks Schedule A server-side on a schedule, caches the aggregated top-N by contributor in Cloudflare KV, and serves the cached result to clients in one round-trip. This is a separate problem from the races.html caching note above — races.html needs server-side caching for request-volume/rate-limit reasons; committee.html Top Committee Contributors needs it for *correctness* on the mega-committees where client-side pagination is architecturally impossible. Same infrastructure pattern, distinct drivers. candidate.html does not hit this limit because candidate committees have bounded incoming committee-to-committee transfer volume. **Status update (2026-04-17):** Session 2 of the bulk data pipeline (above) built the exact analogous pattern for *individual* contributors — bulk data in R2 → DuckDB GROUP BY in GitHub Actions → Cloudflare KV (`fecledger-aggregations` namespace, key `top_contributors:{cmte_id}:{cycle}`). Extending the same pattern to close this committee-contributor gap is now straightforward: the pas2 bulk data is already in R2 (`fec/pas2/{year}/pas2.csv`), `scripts/precompute-aggregations.js` can be extended with a second SQL query against pas2 (key pattern e.g. `top_committees:{cmte_id}:{cycle}`), and the AGGREGATIONS KV binding on Pages is already wired. Roughly a half-session of work to close this gap once Session 3 has shipped the read-side pattern on the frontend.
