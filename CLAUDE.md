...
scripts/
  ...
  precompute-aggregations.js    — DuckDB-based pre-computation of top contributors per committee per cycle. Runs after
                                  ingest-bulk.js completes. For cycles 2024 + 2026: downloads pas2 + indiv + cm CSVs from R2 to /tmp, runs
                                  ...
                                  stale/inaccurate entries from coexisting with fresh data. TTL: 7 days. Schema constants for each file
...
- **Top Committee Contributors — pas2 coverage gaps (unrelated to name source, still open):** The cm.txt integration (Session 4B) resolved the display-name bug but did NOT close two independent gaps in pas2's coverage. (1) **Conduit mega-committees (ActBlue, WinRed):** their inbound Schedule A volume is dominated by `memo_code='X'` rows representing individual contributions forwarded through the platform — these aren't in pas2 at all, so no bulk aggregation solution exists within our current R2 inventory. (2) **National party committees (DNC, RNC, DSCC, NRSC, DCCC, NRCC):** their inbound is dominated by FEC "transfer" transactions between affiliated party committees, which is a distinct transaction type from "contribution" and also isn't captured in pas2. Both classes of committee therefore fall through to the live-API path on committee.html and, being over the 100-page threshold, surface the "Unable to show due to high transaction volume." empty state. Closing either gap requires an additional bulk-data ingest (Schedule A memo rows, or a transfers-specific source) — out of scope here.
