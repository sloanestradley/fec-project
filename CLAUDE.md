...
- **Testing:** Playwright (`@playwright/test`) — `npx playwright test` runs 267 structural tests (mocked API); `npm run test:smoke` runs 5 live-API smoke tests. See `TESTING.md`.
...
**Shared form controls:** `.form-input`, `.form-select`, `.form-search-btn` (and their focus/disabled variants) are defined in `styles.css` and used across search.html, candidates.html, and committees.html. `.toggle-switch` (CSS-only pill toggle: hidden checkbox + `.toggle-track` + `.toggle-knob` + `.toggle-label`) is also defined in `styles.css` — used on committees.html filter bar for "Show terminated". `.toggle-label` uses DM Sans 0.8rem to match form input value text. Page-specific extensions stay inline: `.search-combo .form-input` (flex + border-right), `.state-combo .form-input` (fixed width), `.form-select.wide` (committees only), `.search-bar .form-input` (search.html flex + border-right).
