...
committee.html    — Single committee profile (live, T8 parity with candidate.html: bare URL → index view CareerStrip + cycle index; #{year}#{tab} → detail view with Summary/Raised/Spent tabs + cycle switcher; #cycles → index. All-time mode removed 2026-04-27)
...
### What's working
- **Index view (landing state):** Bare URL (`/candidate/{id}`) → `isDetailView = false` → renders `#career-strip` (4-stat CareerStrip: First Filed / Last Activity / Career Raised / Career Spent) + `#cycle-index` (clickable table of election cycles). No tabs bar, no summary strip, no detail content. Two parallel API calls: `/candidate/{id}/history/?per_page=1` (career dates + `election_years`) and `/candidate/{id}/totals/?per_page=100` (filtered to `election_full: true` rows, keyed by `candidate_election_year`). Cycle labels derived from `coverage_start_date` on each totals row (API-sourced, handles Senate multi-year spans correctly). Cycles below `ARCHIVE_MIN_YEAR[office]` (H: 2008, S: 2012, P: 2012) rendered as non-navigable `div.cycle-row--archive` elements; above threshold rendered as `a.cycle-row` with `href="#year#summary"`. `#cycles` hash → also index view (via `parseInt('cycles') → NaN → ALL_CYCLES.indexOf(NaN) = -1`). Clicking a row triggers `hashchange` → persistent `hashchange` listener calls `switchView(true, cycle)` — in-place transition, no page reload. Back button → bare URL → `hashchange` listener calls `switchView(false, NaN)` → index view re-rendered from cache. Data from the initial index fetch is cached in `cachedHistory` / `cachedAllTotals` module-scoped vars and reused on back navigation (no re-fetch). `history.scrollRestoration = 'manual'` is set at `init()` start to prevent browser auto-scroll on hashchange. Use `election_years` (not `cycles`) from `/history/` response — `election_years` gives proper election groupings (Senate: [2010,2018,2024…]); `cycles` gives all FEC 2-year periods (Senate: [2010,2012,2014,…]). **Compact continuity across transitions:** `compactThreshold` (module var set in `initCompactHeader()`) is the scrollY at which compact mode engages. `switchView(true)` captures `indexScrollY = window.scrollY` before DOM mutations, then scrolls to `compactThreshold` when `indexScrollY >= compactThreshold`. Requires a temporary `.main` `minHeight` before the scroll call — detail view in skeleton state is shorter than viewport so `scrollTo(threshold)` would be clamped to 0; cleared after `loadCycle()` resolves. `initCompactHeader()` is called unconditionally in `init()` so compact works on index-only visits. The scroll listener is the sole owner of `.compact` — `switchView()` never touches it; the listener fires naturally on the programmatic scroll. Tab hash must be captured BEFORE `await loadCycle()` — `loadCycle()` calls `history.replaceState(null,'','#cycle#summary')` internally, overwriting the original `#cycle#raised` hash before `switchView()` can read it.
...
- ~~committee.html~~ ✅ structural parity + T8 index landing state — tabs bar (Summary/Raised/Spent) + cycle switcher, per-cycle stats, overspend callout, title-cased name, relType-aware associated candidate section (`fetchAndRenderAssocSection()` — back-link removed from header, gated behind detail view), .candidate-card-office removed, URL hash encoding (`#{year}#{tab}` for detail; bare URL or `#cycles` for index), `Tab Switched` Amplitude event, CareerStrip (First Filed / Last Activity / Lifetime Raised / Lifetime Spent) + cycle index table on bare URL (T8, 2026-04-27)
