...
**Browse result row pattern (`.candidate-card`, `.committee-row`):** Both use `border-bottom:1px solid var(--border)` only — no background, no full border, no adjacent-sibling `border-top:none` suppression (no longer needed with bottom-border-only). Horizontal padding is `var(--space-8)` on both, giving hover tint breathing room. Hover state is `background:var(--surface2)`. Both use `display:grid; grid-template-columns:1fr auto; align-items:center; gap:var(--space-8) var(--space-16)` — name left, meta tags right. `align-items:center` (changed from `flex-start` on 2026-05-06) keeps the shorter side vertically centered when the other wraps — typical case is a long candidate name wrapping to 2 lines on narrow viewport while the meta tags stay single-line. On mobile (≤860px): grid collapses to `1fr`, `.candidate-card-meta` and `.committee-card-meta` drop below and left-align via `justify-content:flex-start; width:100%`. **`.committee-card-meta`** is the shared meta wrapper for `.committee-row` across all three call sites (candidate.html committees modal, committees.html browse, search.html search results — all rendered via `committeeRowHTML(c, opts)`) — `display:flex; align-items:center; gap:var(--space-8); flex-shrink:0; flex-wrap:wrap; justify-content:flex-end` — comma-grouped with `.candidate-card-meta` in `styles.css`.
