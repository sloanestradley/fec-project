...
- Compact sticky header (`#compact-header`): appears below the top nav when the `#profile-header-row` scrolls behind it; shows `[race label] / [candidate name]` in heading style; populated from the same data as the full header; scroll listener (passive) on `profile-header-row.getBoundingClientRect().bottom <= headerH`; tabs bar `top` offset updated dynamically via `compactEl.offsetHeight`. **Note:** single-element refactor planned (see `plans/adaptive-shimmying-sutton.md`) — current two-element approach has a trigger timing compromise.
