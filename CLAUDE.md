...
- Profile header: `.back-affordance-slot` (T15, 2026-05-11) above `.profile-header-row` carries the "Back to all cycles" icon-only chevron button — visible only on cycle detail view (when `#profile-header` carries `.detail-view`, toggled by `initViewSwitcher`); slot reserves vertical space on cycle index expanded so the title sits at the same y-position across views; slot collapses entirely in compact + cycle-index state; button is 48×48 desktop / 40×40 mobile expanded, 32×32 in compact (both desktop and mobile). `.profile-header-row` below has candidate name + "Committees →" primary-action trigger floating right via `margin-left:auto`; `.meta-row` below the header row (as a sibling inside `.page-header`) carries, in order: `FEC ID · {id}` neutral tag → race tag (`<span class="tag tag-neutral">` with `formatRaceName()` short form, e.g. "House • WA-03") → party tag → incumbent tag (when applicable, cycle-dependent — appended at the end by `loadCycle()` so order stays stable across cycle switches). The long-form race label lives in the race-context-bar's prose-emphasis label below; the meta-row's short tag form matches the race-tag pattern used on candidate cards everywhere else on the site.
