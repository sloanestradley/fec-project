...
**Party tag render order:** Race tag → latest-cycle tag (when present) → party tag — on candidates.html and search.html. This is the canonical order for browse/search results: identity tags first (what office and which cycle), partisan signal last. Rendered by `candidateCardHTML(c, opts)` in utils.js — single source of truth for the order. On candidate.html as of T15 (2026-05-11), the race element is the first child of `.meta-row` (a `<span class="candidate-race-label">` containing the race link), followed by the party tag — profile header has no cycle tag (cycle is in the tabs bar / cycle switcher), so the order is just race → party (→ `.tag-inc` incumbent when applicable → `.fec-id-tag`). Visual mismatch on candidate.html (race rendered at heading-style 1.25rem against 0.625rem mono tags) is acknowledged; a meta-row context variant is pending explicit direction. Party tag always includes `title="..."` via `partyTooltip(c.party, c.party_full)` — native browser tooltip on desktop hover.
...
- Profile header: `.back-affordance-slot` (T15, 2026-05-11) above `.profile-header-row` carries the "Back to all cycles" icon-only chevron button — visible only on cycle detail view (when `#profile-header` carries `.detail-view`, toggled by `initViewSwitcher`); slot reserves vertical space on cycle index expanded so the title sits at the same y-position across views; slot collapses entirely in compact + cycle-index state; button is 48×48 desktop / 40×40 mobile expanded, 32×32 in compact (both desktop and mobile). `.profile-header-row` below has candidate name + "Committees →" primary-action trigger floating right via `margin-left:auto`; `.meta-row` below the header row (as a sibling inside `.page-header`) carries, in order: race link (`.candidate-race-label` — T15 relocation from above title; long-form `formatRaceLabelLong()` in Oswald 400 1.25rem uppercase red-700 — styling unchanged from prior above-title position, pending explicit direction on a meta-row context variant; visual mismatch against the 0.625rem tags is acknowledged) → party tag → incumbent tag (when applicable, cycle-dependent — inserted by `loadCycle()` via `insertBefore` relative to `.fec-id-tag` so order stays stable across cycle switches) → `FEC ID · {id}` neutral tag. Race label href includes `&year=activeCycle` but is not updated on cycle switch — pre-existing behavior, banked for T16 consideration.
