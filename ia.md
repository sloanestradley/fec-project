...
| `races.html` | `/races` | — | `zip`, `year` | **Location search (2c/2d).** ZIP path syncs `?zip=&year=` (shareable, `pushState`); the **address path writes NO URL / history** (geocode-and-discard privacy). A ZIP resolves via `/api/geo/resolve` → the federal races that touch it (per-office `/elections/`). No office/state filters. An out-of-range / odd / yearless `?year=` is **canonicalized** to the resolved cycle via `replaceState` (`canonicalizeUrl`, 2026-07-16), so the URL always reflects the displayed cycle; a stray/invalid `?zip=` on the bare view is stripped. |
