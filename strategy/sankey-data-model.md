...
> **BUILD — Steps 0–5; Gate 1 un-gated, Gate 2 adapter-ready but production-gated (updated 2026-06-10).** Originally research-only (2026-06-04); the build began the same week. **Shipped:** Step 0 (donut field-name fixes), Step 1 (shared `sankey.js` + vendored Apache ECharts 5.5.1), Step 2 (mount on candidate.html), Step 3 (mount on committee.html + form-agnostic adapter), **Step 4 (the donut↔Sankey scope toggle — shipped folded into T-profile-flatten 9c, 2026-06-10): the breakdown slot (`#breakdown-slot`) now renders Money flow Sankey XOR the Raised/Spent donut pair, MUTUALLY EXCLUSIVE on the gate — in-scope → Sankey, gated → donut pair with NO caption (the gate detector only selects the viz). The donuts no longer coexist with the Sankey. A `breakdown_viz` dimension on `Page Viewed` records which mounted.** **Step 5 (2026-06-10) shipped the Gate-1 dual-account un-gate** (§4a Gate 1 — FEA node + non-fed transfers COMMINGLED into "Transfers in" per representation (a), not a separate node + remainder catch-alls; conserves to the penny on WI/TX incl. the $17K residual) **and the Gate-2 presidential ADAPTER** (§4a Gate 2 — the two exempt spend leaves itemized as "Fundraising" + "Exempt legal & accounting"; conserves on modern + publicly-financed presidential records). **Still pending:** the **Gate-2 production flip** is blocked on raised-side Form-3P loan resolution (`loans_received`/`other_loans_received` land in the Other-receipts remainder — the Loans node doesn't read them; needs alias de-dup vs `all_loans_received`), so presidential stays gated → donut fallback. Plus a11y data table, full `.viz-tt` tooltip alignment + the non-fed/FEA disclosure tooltips, color rank-vs-identity, conduit precompute. Because the slot is keyed on `gated` per entity (9c), each un-gate flips its entities donut→Sankey IN PLACE with zero layout rework. This doc remains the durable reference for the FEC data model (§1–§2), the conservation contract (§4), and the v1 gates + ungate paths (§4a); full layout/slot mechanics live in `strategy/profile-flatten-layout.md`. The rendered prototype is `strategy/sankey-examples.html`.
...
- (Similarly, the donut merges the three `offsets_to_*` fields — plus `fed_candidate_contribution_refunds` — into one **"Offsets"** wedge, matching the Sankey's "Offsets" node.)
...
>
> **Un-gate work — ✓ EXECUTED 2026-06-10 (`sankey.js`; verified penny-exact on WI/TX + Playwright sankey.spec.js fixtures):** added to the Sankey an **FEA uses node** (`fed_election_activity`, push-by-presence), **commingled non-fed transfers into "Transfers in"** (`transfers_from_nonfed_account + transfers_from_nonfed_levin`), and switched **both** catch-alls to **remainders** (`receiptTotal/disbTotal − Σ named`) so residuals like TX's $17,063.22 are absorbed (conservation by construction). Kept the `operating_expenditures` parent (no donut double-count). Removed the Gate-1 (non-federal) branch of `sankeyGateReason`. **Commingle vs separate node — DECIDED (fold/commingle):** a mid-build review proposed making non-fed its OWN "Transfers from non-federal account" node (to match the donut wedge + symmetry with FEA). Rejected — it would VIOLATE the locked representation (a) (soft money stays commingled, NOT fed/non-fed-split; the full hard/soft split is the separate scoped viz). The FEA-symmetry argument was a misread: itemizing FEA shows a COMMINGLED parent (its own non-fed child stays inside it), so the parallel move on the raise side is to commingle the transfers, not split them. The donut's separate non-fed wedge is now the orphaned outlier (dead code — dual-account renders the Sankey), flagged for the donut-retirement ticket. **Tooltips (minimal, representation (a)) — deferred render-layer follow-up:** "Transfers in" notes it includes the committee's own non-federal account; "Federal election activity" plain-language GOTV/registration funded partly with non-federal money. **No full hard/soft explainer** (representation (b), the separate viz).
>
...
> - **Ungate path — ADAPTER ✓ EXECUTED 2026-06-10; production gate flip STILL BLOCKED (one newly-found cause).** Done: (1) the two disbursement leaf nodes — `fundraising_disbursements` ("Fundraising") + `exempt_legal_accounting_disbursement` ("Exempt legal & accounting"), itemized; (2) conservation verified on modern (Harris 2024) + publicly-financed (McCain 2008, the $84.1M-federal-funds case) presidential records in sankey.spec.js — **spend side conserves to $0.00 only because the two exempt nodes are modeled**; (4) both added to the §verify-set + sankey.spec.js fixtures. **NEW BLOCKER found while building (raised side):** McCain's $21M loan is filed under `loans_received` / `other_loans_received` (Form-3P names the adapter's **Loans node does NOT read** — it reads `all_other_loans`/`all_loans_received`), so it lands in the **Other-receipts remainder** (conserves, but mislabeled). The Loans node must be extended to coalesce the Form-3P loan field **with alias de-dup** (`loans_received` ≡ `other_loans_received` ≡ `repayments_other_loans` on McCain — adding more than one double-counts; verify vs `all_loans_received` across forms before summing) before the gate flips. Remaining: that raised-side resolution + (3) candidate.html 4-year presidential path end-to-end (live QA — Sankey reads the `election_full` record so likely fine, unverified) → then remove the `isPresidential` branch. The donut fallback covers presidential safely in the interim — no user-facing harm in waiting.
...
| 7 | **Dem Party of Wisconsin Fed** | **C00019331 / 2024** | 3X | **Dual-account (non-federal) — UN-GATED in Step 5 (2026-06-10).** Now conserves to the penny: FEA itemized ($27.4M / 44.5%), non-fed transfers commingled into Transfers in ($35.6M), Other-receipts remainder = $115,361.71. Full record + assertions in sankey.spec.js. |

**Step-5 un-gate fixtures (dual-account — all tripped Gate 1; ✓ all 5 verified penny-exact against the SHIPPED adapter 2026-06-10: WI src/use-resid $0.00 + FEA $27.4M, TX $0.00 + residual absorbed, CA $0.00 + FEA $8.95M, FL $0.00 + FEA $3.31M, EMILY's $0.00 + FEA correctly absent. WI + TX are locked as sankey.spec.js regression fixtures — the two distinct behaviors, FEA-present + the unexposed-residual remainder; CA/FL/EMILY's are more of the same conserving class):**
