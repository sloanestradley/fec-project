...
| C4.b | "Cycle {cycle} ({startYear}–{cycle})" framing | CUT | — | Already in `#stat-cycle` card | `#stat-cycle` stat | |
...
| C5 | "No financial filings found for the {cycle} cycle" | EMPTY-STATE | Cycle-detail empty state (whole-view, not a table cell) | Concept survives as the cycle-detail's empty-state copy | — | |
...
| C8.a | "Raised breakdown from FEC totals endpoint" source | CUT | — | Lacks value at per-viz level; the page-level source line covers attribution | C4.e | |
...
| C8.e | "Coverage through {date}" (raised footer) | CUT | — | Coverage-through date lives at PAGE-NOTE | C4.c | |
...
| C10.a | "Spending by Purpose: keyword mapping…" | CUT | — | Already covered by C9 tooltip | C9 | |
| C10.b | "(capped at 500 transactions)" conditional clause | TOOLTIP-VIZ | Spending by Purpose bars (conditional content within C9's tooltip) | Surfaces only when cap fires; otherwise tooltip carries base methodology | C9 (same tooltip, conditional fragment) | |
| C10.c | "Spending by Category: FEC totals endpoint" source | CUT | — | Lacks value at per-viz level; the page-level source line covers attribution | C4.e | |
...
| K1.b | "{startYear}–{cycleOrAll} cycle" framing | CUT | — | Already in `#stat-cycle` card | `#stat-cycle` stat | |
...
| K3.a | "Raised breakdown from FEC totals endpoint" source | CUT | — | Lacks value at per-viz level; the page-level source line covers attribution | K1.d | |
...
| K9 | "Coverage through {date}" (raised footer) | CUT | — | Coverage-through date lives at PAGE-NOTE | K1.c | |
