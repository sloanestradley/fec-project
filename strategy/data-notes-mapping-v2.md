...
**Placement:** the donut-info ⓘ lives inside the Raised tab, so the actual swap onto the shipped tooltip component is deferred along with the rest of the Raised-tab placement question (see §3 / Sloane's Raised-tab deferral). The audit + this row addition unblock the V1 scoping; the per-row Complete + Verified columns stay empty until the downstream swap ticket lands.
