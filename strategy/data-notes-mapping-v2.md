...
3. **Data-table headers** (Top Conduit Sources — C8.d / K8; Top Vendors — C10.d / K16.d). *Top Individual Contributors and Top Committee Contributors (both pages) get no tooltip — K4–K7 + C8.c all cut 2026-06-01 (§5.j).*
...
**9 live entries** (was 14; rows 3a / 3b / 4a / 4b / 4c cut 2026-06-01 per §5.j). **V1 scope: 8 rows** (1, 2a, 2b, 6a, 6b, 7, 8a, 8b) — of which 2a + 2b shipped 2026-06-01 (choropleth). **Deferred to V2: 1 row** (5 — the conduit-sources explanation, both pages). The V2 set shrank from 6 to 1 because the bulk-vs-API source-attribution family (K4–K7 → rows 3a/3b/4b/4c) AND its candidate twin (C8.c → row 4a) were all cut — **the Top Individual Contributors table and the Top Committee Contributors table (both pages) now get no tooltip at all.** Variants split into letter suffixes for: choropleth (2a base + 2b amendment caveat — shipped 2026-06-01 on BOTH pages per the §5.b parity decision; the "committee-only" framing is retired), Spending by Purpose (6a candidate base / 6b committee base — cap conditional merged inline per fragment in each), and archive divider (8a candidate / 8b committee). When these copies are mounted onto the component, each row's audit ID flips to ✓ in the §1 mapping table.
