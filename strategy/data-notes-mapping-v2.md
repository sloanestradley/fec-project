...
| K3.b | "Geography reflects itemized individual contributions by state (Schedule A)" methodology | TOOLTIP-VIZ | Where Individual Contributions Come From map | **Shipped 2026-06-01** (T-choropleth-tooltip). Mounted on the map section title (same tooltip as K3.c); both sentences removed from the `#raised-data-note` footer (footer now carries only the K4–K8 Top-Contributor clauses). | Parity with C8.b — also receives K3.c content | ✓ | ✓ |
...
**14 total entries** (8 distinct attachment surfaces). **V1 scope: 8 rows** (1, 2a, 2b, 6a, 6b, 7, 8a, 8b) — surfaces with simpler placement (stat label, choropleth, Spent-tab bars/table without nested sub-tabs, cycle-index archive divider). **Deferred to V2: 6 rows** (3a, 3b, 4a, 4b, 4c, 5) — all inside the Raised → "Top Contributors by type" nested sub-tab section (Committees / Conduits / Individuals); tooltip-on-sub-tab placement is more complex and benefits from its own scoping pass. Variants split into letter suffixes for: choropleth (2a base + 2b amendment caveat — shipped 2026-06-01 on BOTH pages per the §5.b parity decision; the "committee-only" framing is retired), Top Individuals (3a bulk / 3b API conditional swap), Top Committees (4a candidate / 4b committee bulk / 4c committee API), Spending by Purpose (6a candidate base / 6b committee base — cap conditional merged inline per fragment in each), and archive divider (8a candidate / 8b committee). When these copies are mounted onto the component, each row's audit ID flips to ✓ in the §1 mapping table.
