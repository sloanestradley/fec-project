...
> **EXECUTED 2026-05-28 → 2026-06-01. Closed out.** Every mapping row is resolved — each concept is either mounted, CUT, or n/a. Mounted destinations: PAGE-NOTE on candidate + committee; TOOLTIP-VIZ on the donut wedges (C13/K18), choropleth, Spending by Purpose, cycle-index archive divider, Raised:Spent ratio (C4.d), and the Conduit column header (C8.d/K8); SECTION-NOTE for the committees-modal JFA gap (C11); `.callout` for overspend (C7/K2). The final two rows closed in the 2026-06-01 session: **C8.d / K8** (conduit-source explanation → Conduit column-header tooltip; footer clause retired and the now-dead `#raised-data-note` element removed on both profile pages) and **R1.a / R1.b** (race.html `#race-note` CUT entirely — race.html isn't yet rich enough to warrant a page-level data note, a deliberate divergence from candidate/committee; revisit if it gains depth). **§1 (the tooltip mapping) is fully closed.** The one banked-but-open item — the primary-loser / roster-composition caveat on race (§5.e) — was never carried by a mapping row; it's a product-honesty question gated on a primary-results data source the FEC doesn't provide. Doc retained as the historical record of the audit → mapping → execution discipline.
...
3. **≤$200 itemization caveat (C8.f / K10).** Shipped at PAGE-NOTE level 2026-05-29, then **relocated 2026-06-11 to the Itemized Individual Contributions Map tooltip** (copy: *"Contributions of $200 or less are not required to be itemized."*) — it sits with the itemized map it qualifies, and #page-note was slimmed to coverage-only. Applies on candidate and committee.
...
The tooltip component **shipped 2026-05-29 in T-tooltip-component** (see CLAUDE.md `initTooltips` entry under Shared files). The tooltip COPIES from §1 were then mounted onto their target surfaces across 2026-06-01 — **the last one (V2 row 5, conduit-sources / C8.d / K8) shipped 2026-06-01, completing the mounting work.** Spec input for that mounting work lived in §3.
...
| C4.d | "Raised-to-spent = total receipts ÷ total disbursements" definition | TOOLTIP-LABEL | Raised:Spent Ratio stat label | **Shipped 2026-06-01** (the definition was cut from #data-note in Phase 2; now mounted as a TOOLTIP-LABEL on the ratio stat card, candidate-only). **Positioning (verified):** the `.stat-card--has-info` card is `position:relative` `display:block`; the `.tooltip` **host** is `position:absolute` (`top/right:var(--space-8)`) which lifts it out of flow so it can't grow the card or its shared grid-row height. The `.tooltip-trigger` **button** inside the host is `position:static` (it fills the host) — so inspecting the button shows `static`, which is expected; the positioning is the host's. Wired by the init-time `initTooltips()` (static markup). Copy: *"Total receipts ÷ total disbursements"*. | — | ✓ | ✓ |
...
| C7 | Overspend callout | (stays as `.callout`) | Near the timeline chart on the summary tab | Concept survives in its current visible-callout form | Judgment call resolved: option (a). **Relocated 2026-06-11** from #page-note to the Itemized Individual Contributions Map tooltip (copy: *"Contributions of $200 or less are not required to be itemized."*); #page-note slimmed to coverage-only and "Source: FEC." retired in favor of the global banner. Not absorbed into the data-note family. | n/a | n/a |
...
| C8.d | Conduit-source explanation (memo_code=X, individuals' money not platforms') | TOOLTIP-VIZ | Conduit column header, Top Conduit Sources table | **Shipped 2026-06-01.** Mounted on the "Conduit" column header (renamed from "Committee (Conduit)") as static `.tooltip` markup wired by the init-time `initTooltips()`. Copy rewritten — dropped the memo_code=X mechanics for audience-appropriate phrasing: *"Conduits are platforms like ActBlue and WinRed that forward contributions from individual donors. The amounts shown reflect money from those donors, not the platform itself."* The C8.d footer clause was retired and the now-dead `#raised-data-note` element removed. | — | ✓ | ✓ |
...
| C8.f | "Small-dollar contributions ≤$200 are not itemized" | PAGE-NOTE | Page-level note | Shipped 2026-05-29 (Phase 2 candidate+committee). Sentence moved from #raised-data-note to #page-note (page-level, applies across all individual-contribution surfaces — donut, choropleth, summary totals). Revised copy: *"Individual contributions of $200 or less are not itemized."* | Parity with K10 — same PAGE-NOTE entry across candidate + committee. Judgment call resolved: option (a). **Relocated 2026-06-11** from #page-note to the Itemized Individual Contributions Map tooltip (copy: *"Contributions of $200 or less are not required to be itemized."*); #page-note slimmed to coverage-only and "Source: FEC." retired in favor of the global banner. | ✓ | ✓ |
| C9 | "Categories estimated from FEC disbursement descriptions using keyword matching. Some transactions may be miscategorized. Covers most recent sub-cycle." | TOOLTIP-VIZ | Spending by Purpose bars | **Shipped 2026-06-01.** Mounted on the "Spending by Purpose" section title via `initTooltips`, rebuilt per spent-render (dynamic) so the C10.b cap fragment tracks fetch state; the inline `.data-note` under the bars was removed. "Most recent sub-cycle only" clause is candidate-only. **Copy tightened 2026-06-01** — dropped "FEC" before "disbursement descriptions"; candidate "sub-cycle." → "sub-cycle only." (K14 committee copy dropped "FEC" too). | Consolidates C10.a | ✓ | ✓ |
...
| C10.b | "(capped at 500 transactions)" conditional clause | TOOLTIP-VIZ | Spending by Purpose bars (conditional content within C9's tooltip) | **Shipped 2026-06-01.** Now wired — appended to the C9 tooltip when `spentData.capped` (fetchSpentData hit MAX_PAGES=5). Copy lowercased to a parenthetical aside *"(capped at 500 transactions)"* (Sloane's call — was the sentence-style "(Capped at 500 transactions.)"). | C9 (same tooltip, conditional fragment) | ✓ | ✓ |
...
| C12.b | "FEC coverage begins {threshold} for {office} races" explanation | TOOLTIP-VIZ | Cycle-index archive divider (anchored beside C12.a's label) | **Shipped 2026-06-01.** Mounted as a `.tooltip` beside the divider label via a shared `archiveDividerHTML(office)` helper (one source for all 3 render sites — index / scaffold / partial-error); `initTooltips(container)` after each cycle-index render. Copy: *"No detail view available for {House\|Senate\|Presidential} races prior to {ARCHIVE_MIN_YEAR[office]}"* `.cycle-archive-divider` gained `height:32px` (+ no gap) so the 32px trigger doesn't grow the band. | — | ✓ | ✓ |
...
| C13.c | Methodology for "Other receipts" wedge | TOOLTIP-VIZ | Raised-tab donut legend, "Other receipts" wedge | Production verbatim: *"Miscellaneous funds (e.g., interest on bank accounts, dividends, and similar incidental income)"* (copy revised 2026-06-01) **Shipped 2026-06-01** (donut-tooltip swap; copy verbatim, review pending). | — | ✓ | ✓ |
| C13.d | Methodology for "Loans" wedge | TOOLTIP-VIZ | Raised-tab donut legend, "Loans" wedge | Production verbatim: *"Loans from banks or other third parties (not including candidate self-funding loans)"* (copy revised 2026-06-01) **Shipped 2026-06-01** (donut-tooltip swap; copy verbatim, review pending). | — | ✓ | ✓ |
| C13.e | Methodology for "Federal funds" wedge | TOOLTIP-VIZ | Raised-tab donut legend, "Federal funds" wedge | Production verbatim: *"Public financing from the Presidential Election Campaign Fund"* (copy revised 2026-06-01 — the "Only available to presidential candidates" clause was dropped as redundant: $0 categories are never rendered (`active = cats.filter(vals[i] > 0)`, candidate.html:1007), so the Federal funds wedge only ever appears for presidential candidates who received public financing — a non-presidential filer never sees it) **Shipped 2026-06-01** (donut-tooltip swap; copy verbatim, review pending). **Definitional / contextual** (was mislabeled interpretation-critical — corrected 2026-06-01; the "$0 expected" rationale assumed an empty wedge could render, but $0 wedges never do. See §5.i). | — | ✓ | ✓ |
...
| K2 | Overspend callout | (stays as `.callout`) | Near the chart on the summary tab | Concept survives in its current visible-callout form | Judgment call resolved: option (a). **Relocated 2026-06-11** from #page-note to the Itemized Individual Contributions Map tooltip (copy: *"Contributions of $200 or less are not required to be itemized."*); #page-note slimmed to coverage-only and "Source: FEC." retired in favor of the global banner. Parity with C7. | n/a | n/a |
...
| K8 | Conduit-source explanation (memo_code=X, individuals' money not platforms') | TOOLTIP-VIZ | Conduit column header, Top Conduit Sources table | **Shipped 2026-06-01.** Parity with C8.d — same column rename, same static tooltip + rewritten copy, K8 footer clause retired and `#raised-data-note` element removed. | Parity with C8.d | ✓ | ✓ |
...
| K10 | "Small-dollar contributions ≤$200 are not itemized" | PAGE-NOTE | Page-level note | Shipped 2026-05-29 (Phase 2 candidate+committee). Sentence moved from #raised-data-note to #page-note (page-level). Revised copy: *"Individual contributions of $200 or less are not itemized."* | Parity with C8.f — same PAGE-NOTE entry across candidate + committee. Judgment call resolved: option (a). **Relocated 2026-06-11** from #page-note to the Itemized Individual Contributions Map tooltip (copy: *"Contributions of $200 or less are not required to be itemized."*); #page-note slimmed to coverage-only and "Source: FEC." retired in favor of the global banner. | ✓ | ✓ |
...
| K14 | "Categories estimated from FEC disbursement descriptions using keyword matching. Some transactions may be miscategorized" | TOOLTIP-VIZ | Spending by Purpose bars | **Shipped 2026-06-01.** Parity with C9 — same dynamic mount on the section title + inline-note removal; committee omits the "most recent sub-cycle" clause. | consolidates K16.a | ✓ | ✓ |
...
| K16.b | "(capped at 500 transactions)" conditional clause | TOOLTIP-VIZ | Spending by Purpose bars (conditional content within K14's tooltip) | **Shipped 2026-06-01.** Now wired — appended when `data.capped`. Lowercased parenthetical *"(capped at 500 transactions)"*. Parity with C10.b. | K14 (same tooltip, conditional fragment) | ✓ | ✓ |
...
| K17.b | "FEC coverage begins {threshold}" explanation | TOOLTIP-VIZ | Cycle-index archive divider (anchored beside K17.a's label) | **Shipped 2026-06-01.** Mounted via a shared `archiveDividerHTML()` helper (3 render sites); `initTooltips(container)` after each render. Copy: *"No detail view available for cycles prior to 2008"* (threshold = `ARCHIVE_MIN_YEAR_COMMITTEE` = 2008; committee omits the {office} clause). | — | ✓ | ✓ |
...
| K18.c | Methodology for "Other receipts" wedge | TOOLTIP-VIZ | Raised-tab donut legend, "Other receipts" wedge | Production verbatim (identical to C13.c): *"Miscellaneous funds (e.g., interest on bank accounts, dividends, and similar incidental income)"* (copy revised 2026-06-01) **Shipped 2026-06-01** (donut-tooltip swap; copy verbatim, review pending). | Twin of C13.c | ✓ | ✓ |
| K18.d | Methodology for "Loans" wedge | TOOLTIP-VIZ | Raised-tab donut legend, "Loans" wedge | Production verbatim (identical to C13.d): *"Loans from banks or other third parties (not including candidate self-funding loans)"* (copy revised 2026-06-01) **Shipped 2026-06-01** (donut-tooltip swap; copy verbatim, review pending). | Twin of C13.d | ✓ | ✓ |
| K18.e | Methodology for "Federal funds" wedge | TOOLTIP-VIZ | Raised-tab donut legend, "Federal funds" wedge | Production verbatim (identical to C13.e): *"Public financing from the Presidential Election Campaign Fund"* (copy revised 2026-06-01 — the "Only available to presidential candidates" clause was dropped as redundant: $0 categories are never rendered (`active = cats.filter(vals[i] > 0)`, candidate.html:1007), so the Federal funds wedge only ever appears for presidential candidates who received public financing — a non-presidential filer never sees it) **Shipped 2026-06-01** (donut-tooltip swap; copy verbatim, review pending). **Definitional / contextual** (same correction as C13.e — $0 wedges never render, so there's no "$0 reading" to clarify). See §5.i. | Twin of C13.e | ✓ | ✓ |
...
| Audit ID | Concept | Destination | Specific attachment point | What's preserved | What's cut / merged with | Complete | Verified |
|---|---|---|---|---|---|---|---|
| R1.a | "as reported to the FEC" loose source attribution | ~~PAGE-NOTE~~ → CUT | — | Shipped 2026-05-28 as a page-level source line (*"Source: FEC."* → fec.gov), then **CUT entirely 2026-06-01** along with the whole `#race-note` element. Sloane's call: race.html isn't yet rich enough to warrant a page-level data note, including a data source — a deliberate divergence from candidate.html / committee.html, which keep `#page-note`. Revisit if race.html gains depth. | — | ✓ | ✓ |
| R1.b | "Candidates may not yet have filed for all periods" data-completeness caveat | ~~PAGE-NOTE~~ → CUT | — | Shipped 2026-05-28, then **CUT entirely 2026-06-01** with the rest of `#race-note` (see R1.a). The roster-composition gap (§5.e) is unaffected — it was always a separate open product question, not carried by this note. | — | ✓ | ✓ |
...
- **Shortest (TOOLTIP-LABEL on stats):** single-sentence definitions, ~10–20 words. E.g. C4.d *"Total receipts ÷ total disbursements"*.
...
- **C9** — "Covers most recent sub-cycle only" trailing clause is candidate-only (Senate sub-cycle artifact); committee equivalent K14 omits it.
...
|---|---|---|---|---|---|---|---|
| 1 | ✓ | *"Total receipts ÷ total disbursements"* | Raised:Spent Ratio stat-card label (TOOLTIP-LABEL) | candidate | definitional / contextual | C4.d | **Shipped 2026-06-01.** Positioned via the absolute `.tooltip` host (top-right of card); the trigger button inside it is `position:static`. |
| 2a | ✓ | *"Reflects itemized individual contributions by state."* (copy tightened 2026-06-01 — dropped "Geography" + "(Schedule A)") | Where Individual Contributions Come From map | both | definitional / contextual | C8.b / K3.b | **Shipped 2026-06-01** (T-choropleth-tooltip). |
...
| 5 | ✓ | *"Conduits are platforms like ActBlue and WinRed that forward contributions from individual donors. The amounts shown reflect money from those donors, not the platform itself."* (copy rewritten 2026-06-01 — the memo_code=X mechanics dropped for audience-appropriate phrasing) | Conduit column header (renamed from "Committee (Conduit)"), Top Conduit Sources table | both | **interpretation-critical** (the architecture's stress case — without it, users misread platform contributions as platform donations; see §5.c) | C8.d / K8 | **Shipped 2026-06-01.** Static `.tooltip` on the column header, wired by the init-time `initTooltips()`; footer clause retired + `#raised-data-note` removed on both pages. |
| 6a | ✓ | *"Categories estimated from disbursement descriptions using keyword matching. Some transactions may be miscategorized. Covers most recent sub-cycle only."* — **when the Schedule B fetch caps**, the tooltip appends *"(capped at 500 transactions)"* (C10.b conditional fragment; trailing "Covers most recent sub-cycle only" is candidate-only Senate sub-cycle artifact) | Spending by Purpose bars on candidate.html | candidate | interpretation-critical (keyword-matching imprecision affects how user reads category totals; the cap append flags incomplete coverage) | C9 + C10.b (conditional) | **Shipped 2026-06-01** (copy tightened — dropped "FEC", "sub-cycle" → "sub-cycle only"). |
| 6b | ✓ | *"Categories estimated from disbursement descriptions using keyword matching. Some transactions may be miscategorized."* — **when the Schedule B fetch caps**, the tooltip appends *"(capped at 500 transactions)"* (K16.b conditional fragment) | Spending by Purpose bars on committee.html | committee | interpretation-critical (keyword-matching imprecision; cap append flags incomplete coverage) | K14 + K16.b (conditional) | **Shipped 2026-06-01** (copy tightened — dropped "FEC"). |
...
| 8a | ✓ | *"No detail view available for {office} races prior to {threshold}"* | Cycle-index archive divider, anchored beside C12.a's label | candidate | definitional / contextual | C12.b | **Shipped 2026-06-01.** |
| 8b | ✓ | *"No detail view available for cycles prior to {threshold}"* | Cycle-index archive divider, anchored beside K17.a's label | committee | definitional / contextual | K17.b | **Shipped 2026-06-01.** |

**8 live entries** (was 14; rows 3a / 3b / 4a / 4b / 4c / 7 cut 2026-06-01 per §5.j). **V1 scope: 7 rows** (1, 2a, 2b, 6a, 6b, 8a, 8b) — **all shipped 2026-06-01.** ✅ **V1 is complete:** donut wedges (C13/K18), ratio stat label (row 1 / C4.d), choropleth (2a/2b), Spending by Purpose (6a/6b), archive divider (8a/8b). **V2 row 5 (conduit-sources explanation, both pages) shipped 2026-06-01 — every §1 tooltip row is now either mounted or CUT, closing §1 entirely.** The cut family: the bulk-vs-API source-attribution rows (K4–K7 → 3a/3b/4b/4c), its candidate twin (C8.c → 4a), and the Top Vendors dedup note (C10.d / K16.d → row 7) were all cut as thin/no-value source-or-scope notes — **the Top Individual Contributors, Top Committee Contributors, and Top Vendors tables (all, both pages) now get no tooltip.** Variants split into letter suffixes for: choropleth (2a base + 2b amendment caveat — shipped 2026-06-01 on BOTH pages per the §5.b parity decision; the "committee-only" framing is retired), Spending by Purpose (6a candidate base / 6b committee base — cap conditional merged inline per fragment in each), and archive divider (8a candidate / 8b committee). When these copies are mounted onto the component, each row's audit ID flips to ✓ in the §1 mapping table.
...
| **Data-completeness caveats** | ≤$200 itemization shipped to PAGE-NOTE 2026-05-29 (Phase 2 candidate+committee); JFA gap (SECTION-NOTE on committees) | ≤$200 itemization shipped to PAGE-NOTE 2026-05-29; volume-cap (EMPTY-STATE) | One caveat: "candidates may not yet have filed for all periods" (R1.b) | R1.b shipped 2026-05-28, then **CUT 2026-06-01** with the whole `#race-note` (race.html isn't rich enough to warrant a page-level note yet). The primary-loser-counting caveat (banked in project-brief.md) remains an open content question — see §5.e — independent of R1.b's removal. | race.html now carries no page-level data note (deliberate divergence from candidate/committee); the primary-loser caveat is a banked-but-open product-honesty question for the designer. | n/a | n/a |
...
- **Tooltip (TOOLTIP-VIZ) — shipped 2026-06-01:** candidate = *"No detail view available for {House\|Senate\|Presidential} races prior to {ARCHIVE_MIN_YEAR[office]}"*; committee = *"No detail view available for cycles prior to 2008"*. Same shape, candidate retains the office-specificity the committee version doesn't need. (Copy revised from the v1 "Data for … is less complete; no detail view available." to lead with the "no detail view" point and drop the "is less complete" clause — Sloane's call.)
...
**Interpretation-critical wedges:** Originally flagged two per page (Federal funds + Refunds & offsets). **Corrected 2026-06-01:** Federal funds is NOT interpretation-critical — the original "$0 is the expected value, the empty wedge could read as 'missing'" rationale was based on a false premise. $0 categories are never rendered (`active = cats.filter(vals[i] > 0)`, candidate.html:1007), so the Federal funds wedge only appears for presidential candidates who received public financing; a non-presidential filer never sees the wedge (or its tooltip) at all. It's now **definitional / contextual** like the others. The remaining genuinely interpretation-critical one:
- **Refunds & offsets (C13.f / K18.f):** clarifies that the wedge represents money credited back to the committee, NOT new money raised. Matters for any "total raised" reading. (This wedge CAN carry a non-zero value, so it does render and the caveat is load-bearing.)

The remaining wedges (C13.a–e / K18.a–e) are definitional / contextual — they explain what each bucket represents without the same kind of interpretive load.
...
**C8.c / row 4a (candidate Top Committee Contributors) — also CUT 2026-06-01.** It was the candidate twin of the cut K6/K7, living in the candidate footer: *"Top committee contributors: complete PAC, party, and other committee contributions across {cycleLabel}, deduplicated by committee ID."* Mostly scope-restatement (cycle is in the title) plus a thin "deduplicated by committee ID" note. Cut for parity (Sloane's call) so neither page's Top Committee Contributors table carries a source/scope tooltip; the clause was removed from the candidate `#raised-data-note`, which at that point carried only the C8.d conduit explanation. **Top Vendors note (C10.d / K16.d, row 7) — also CUT 2026-06-01.** Same thin dedup-methodology shape; removed from both pages' `#spent-data-note` (now empty + hidden). With this, every source-attribution / thin-scope footer note across the Raised and Spent tabs is gone. **The last Path-Z holdout — the C8.d / K8 conduit explanation — shipped as a column-header tooltip 2026-06-01 (V2 row 5); its footer clause was retired and the now-empty `#raised-data-note` element removed on both pages. No Path-Z footer methodology remains anywhere.**
