...
- **Caption A → show single-state too ("both") — AFTER 2e.** Today the "No Senate race in {State} this cycle" note renders **only in multi-state grouped results** (where the neighbor-state contrast makes a correct absence look like a bug); single-state absent offices silently drop (decision #3). **Sloane's call (2026-07-15):** make it consistent — show the note on single-state results as well. ~One-line change: drop the `multiState` gate on Caption A in the progressive-resolve null branch (a null Senate renders `senateOmissionNote` regardless of layout). On build: confirm the line reads well standalone on a single-state result, and that it doesn't clutter the President-only / DC cases (where there's no Senate slot at all — the gate should still be office-aware, not "note on every result"). **Sequenced after the 2e test net** so the flow spec covers both single- and multi-state Caption A paths in one pass.
