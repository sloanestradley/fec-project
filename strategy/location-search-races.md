...
- **(A) Cycle-aware Senate-omission caption — ships LIVE.** A Senate whose `/elections/` returned empty renders **"No {State} Senate race in {year}"** instead of vanishing. It's **derivable now** (read the filing signal the FEC layer already produces — no term-class data needed), and its whole purpose is to make the silent omission explicit, so shipping it dark would defeat it. **Wording describes the data, never asserts seat class:** empty `/elections/` means no race is *listed*, not that the seat isn't *up* — deriving "seat isn't up" from a filing signal is the same unsafe inference shape as `SG`+`G`→two-contests (and the true wording also catches specials that class data would miss). **Scope history:** shipped multi-state-only in 2d (reintroducing the copy decision #3 had removed from the single-state path), then **extended to BOTH layouts 2026-07-16** — grouped → end of the state's group, flat → end of the list; the year became explicit (was "this cycle") since the note now carries alone on a flat result. Senate-only: House never nulls, and a null President is a data anomaly rather than "not up".
...
- **Seat-status contract (2a output) — SUPERSEDED 2026-07-16; see "Seat status → structured + mark-the-exception" below. The 2026-07-08 version is kept for lineage:**
  ...
  - Incumbent test = `incumbent_challenge_full === 'Incumbent' || incumbent_challenge === 'I'` (dual-field, per race.html). **[still true]**
  - **Distinct-incumbent count dedupes by normalized name** — a duplicated incumbent record (same person, two `candidate_id`s in the un-deduped payload) must read as ONE incumbent, NOT "Multiple". Dedupe applies ONLY to the 1-vs-2+ count; the **`total` sums all rows un-deduped** (surface what FEC reports). **[still true]**
  - **Race object shape:** `{office, state, district?, seatStatus, total, href}` — the candidate array is used to derive seatStatus + total, then **dropped** (not retained/handed to race.html, which re-fetches independently). `href` → race.html (the load-bearing handoff; the descope only works because the detail path survives). **[still true — but `seatStatus` is now an object, see below]**
  - **Collapsed-Senate:** the combined card's `total` sums BOTH contests and seatStatus reads `multiple` — both honest under the caption.

- **Seat status → structured + mark-the-exception (2026-07-16, supersedes the above):**
  - **`raceSeatStatus` returns `{ kind, name? }`, not a display string** — `{kind:'none'}` / `{kind:'open'}` / `{kind:'multiple'}` / `{kind:'incumbent', name}`. The branch logic (dedup, dual-field test) is unchanged; only the return shape is.
  - **Why:** `kind` is the stable, low-cardinality fact. Display copy and the Amplitude `seat_status` value now derive from it *independently*, so renaming a label can't silently rewrite the analytics enum — which it could before, because the click event read the **rendered DOM text**. It also makes re-adding the incumbent name a render-only change (`name` already flows).
  - **Tile render (mark-the-exception):** `open` → `.tag.tag-neutral` "Open seat"; `multiple` → `.tag.tag-neutral` "Multiple incumbents"; `none` → plain meta text **"No candidate filings"** (renamed from "No candidates reported" — it names the *filing signal*, matching the Caption A constraint; and an absence of data isn't a race attribute, so deliberately not a tag); **`incumbent` → nothing at all.**
  - **Why the incumbent name left the tile (Sloane, 2026-07-16):** "Incumbent: X" and the total were two identically-styled spans in a flex row, so they read as a subject-predicate pair — implying the total was that candidate's. But `total` is the **race-wide un-deduped sum of every candidate's receipts**, so the card was implying a false attribution. Removing the name also stops a race-level card from over-emphasising one person when it says so little about the race. **Absence of a chip is now the only incumbency signal, by design.** Adding names back is explicitly on the table later.
  - **Accepted trade:** `.tag-neutral`'s background is `--surface-2`, identical to `.race-card:hover`, so the chip flattens into the row on hover (same as `.candidate-card`'s tags). Accepted 2026-07-16.
  - **Analytics:** `seat_status` reads the card's `data-seat-kind` attribute → `'incumbent'|'open'|'multiple'|'none'`, or `null` when a tile is clicked before it resolves (no attribute yet). Breaking value-format change from the 2d strings; ~1 day of history, so migration was a no-op.
  - **`resolveRaces()` deleted** in the same change — a Promise.all convenience "for tests / non-progressive callers" that never acquired a consumer.
...
2. **✅ DONE 2026-07-16 — `GEO_CACHE` KV namespace** — namespace `fecledger-geo-cache` created + bound as `GEO_CACHE` (Production) in Cloudflare dashboard → Pages → fecledgerapp → Settings → Functions → KV bindings; verified live (a `geo:zip:98604:120` key appeared after a prod resolve). **Gotcha banked from the setup:** the binding does NOT apply to already-deployed builds — it needs a *redeploy* after binding (an empty commit to main, or dashboard Retry deployment). Pages KV bindings **can't be set via wrangler** — same manual step as `AGGREGATIONS`. **NOT required for local dev or Stage-1 correctness** — `wrangler pages dev` simulates KV empty → every resolve takes the live path (the Function fails open when unbound), so local dev / Stage-1 verification run without it.
...
- **ZIP-value in Amplitude (D4)** — the Amplitude events **shipped in 2d** (`Location Search` / `Race Tile Clicked` / `Location Search Error`) and log **`input_type` + result shape only, never the ZIP string or address** (privacy honored, grep-verified). What stays **deferred** is whether to *additionally* log the **ZIP value** (a coarser "which areas are searched" signal) — revisit as a privacy-vs-signal call, **leaning never-log**. (Locked 2026-07-09.) Address is never logged under any decision (geocode-and-discard invariant).

### Deferred features (post-v1)
- **Complete City search** — **(City input isn't in v1 — ZIP + address only; City was deferred to ride with the ZIP precompute, so this is the eventual complete-City *build*, not a fix to an already-shipped centroid search.)** Make City+State span-complete (all districts a city touches), not centroid-only. Gated on the ZIP precompute: city → ZIPs (static city→ZIP table) → union of cached districts. Adds one dataset + union logic; accepts USPS-postal-city breadth. (§4 analysis; resolves the City half of open item #6.)
...
- **Senate two-race split** — when a state runs a regular + special Senate election in one cycle, `/elections/` collapses both into one mashed candidate list (GA-2020 verified). Splitting needs per-candidate special-vs-regular attribution the endpoint doesn't provide. Tie to the existing Senate term-class open question rather than solving inside this work. v1 documents + accepts. **NB:** Caption B (the collapse *caption*) **shipped dark in 2d** (`SENATE_COLLAPSE_ENABLED=false`, gated on the term-class dataset) — that only *labels* the collapse ("two elections shown together"); the actual per-contest SPLIT (this item) is separate and needs the same term-class attribution to enable both.

- **✅ RESOLVED 2026-07-16 — Caption A → "both" (single-state too).** Shipped: the note now renders in **both** layouts (grouped → end of the state's group; flat → end of the list, the same "after this state's races" placement), and the copy became **"No {State} Senate race in {year}"** — the year is explicit because the note has to carry alone on a flat result, without the group header or neighbour-state contrast. **Two corrections to the plan below:** (a) the "~one-line change: drop the `multiState` gate" was **wrong — a no-op**: with the gate dropped, a flat result still hits `el.closest('.race-state-group')` → null → the note is never inserted, so a real flat-layout insertion path was needed (`(group || list).insertAdjacentHTML('beforeend', …)`); (b) the office-awareness worry was **already structurally satisfied** — the branch is gated on `item.office === 'S'` and `planRaces` never plans a Senate for DC (skipped) or territories (empty plan), so no extra gate was needed. **(Historical plan)** Today the note renders only in multi-state grouped results (where the neighbor-state contrast makes a correct absence look like a bug); single-state absent offices silently drop (decision #3). **Sloane's call (2026-07-15):** make it consistent. Sequenced after the 2e test net so the flow spec covers both Caption A paths in one pass.
...
- **✅ RESOLVED 2026-07-16 — `?year=` clamp + URL canonicalization.** The deep-link paths now reconcile the address bar with what actually rendered. `canonicalizeUrl()` (races.html) `replaceState`s (no history entry) to the canonical `?zip&year` when a ZIP resolved, or strips params when it didn't — called at the end of both `init()` and `onPopState` (after the overlay no-op guard, so an overlay open/close never triggers it). Effects: a clamped `?year=` (out-of-range / odd / below the 2012 floor / absent → default cycle) gets rewritten to the resolved cycle; a **yearless `?zip=` deep-link is pinned** to the resolved cycle in the URL (note: still the *live* current cycle at open time — the bare page + a yearless link always compute the cycle from the real date, so nothing freezes; only a link *copied from the address bar* now carries the year it was viewed at); a stray/invalid `?zip=`/`?year=` on the bare view is cleared. **Deliberately NOT nearest-bound clamping** — the resolved year is the honest current-cycle fallback, not an invented nearer year (mirrors the geo Function's reject-don't-clamp rule). **(Historical)** Before this, `init()`'s `validYear()` guard failed any invalid year and left `yearParam` at the default current cycle, but the URL kept the typed year (e.g. `?year=2008`) — nothing false *displayed* (the page agreed on the cycle), the URL was merely stale, self-healing on the next interaction. Tests: 5 in `races.spec.js` (out-of-range clamp, odd clamp, yearless-zip pin, valid-year-untouched, invalid-zip-strips) using the time-proof invariant "URL year == `#year-select` value". Related but SEPARATE: the `/elections/search/` phantom-race note below is the same silent-substitution class one layer down (race.html snapping a bad cycle to the nearest listed one) — still banked.

### 2e — ✅ COMPLETE 2026-07-15

**2c + 2d are live** (2026-07-15, commits 96ce799 / d4dc0d1); the doc half + the test net are now both done. The test net shipped as:

1. ✅ **`tests/helpers/geo-mock.js`** — `mockGeoResolve(page)` intercepts `/api/geo/resolve`, returns the NORMALIZED resolve object keyed by `q`, computed cycle-faithfully (President in presidential cycles; future cycle > 2026 → state-only degrade, House dropped) + typed `{error}` for not_found/low_accuracy/cycle_out_of_range. Corpus: 98604 (single), 42223 (multi-state), 60629 (multi-district), 20001 (DC), 00901 (PR territory), 00000 (not_found), a rooftop address + a low-accuracy address.
2. ✅ **`tests/races.spec.js`** (13 tests) — full mocked resolve→render flow with geo-mock + a races-local office/state/cycle-aware `/elections/` override (layered over `mockFecApi`) + amp-mock: flat vs grouped (President-ungrouped-top, alphabetical states), numerical districts, Caption A (both layouts as of 2026-07-16 — grouped end-of-group + flat end-of-list — plus a Senate-only guard), future degrade (no House), territory/DC/not_found/low_accuracy, URL sync (zip pushes / address writes nothing), progressive skeleton→resolve, and the Amplitude events **including the privacy invariant — the searched ZIP appears in NO event's serialized args** (asserted on both `Location Search` and `Race Tile Clicked`).
3. ✅ **`tests/geo-normalize.spec.js`** (10 tests) — `normalize` was exported from the Function (inert in production — Pages only treats `onRequest*` as handlers) and imported directly (ESM); covers single/multi-district, multi-state union, at-large (current + historical), DC, territory, presidential-office, the future-degrade shape (no `congressional_districts` → no House), and the ≥90 delegate-district skip.
...
**Banked but NOT 2e (separate follow-ons — see "Known defects" below):** the `/elections/search/` contest-existence gate (phantom races); ~~`toTitleCase` honorific/suffix mangling (site-wide)~~ **RESOLVED 2026-07-15** (see Known defects entry); ~~the `?year=` deep-link clamp/canonicalize~~ **DONE 2026-07-16** (see Known defects entry); ~~the optional `GEO_CACHE` KV production binding~~ **DONE 2026-07-16** (namespace `fecledger-geo-cache`, verified live).

### Known defects surfaced by the location-search arc

- **✅ RESOLVED 2026-07-15 — `toTitleCase` honorific/suffix hardening (site-wide).** `toTitleCase` now strips embedded honorifics (`MR/MRS/MS/MISS/MISTER/DR/HON/REV`) and relocates a trailing generational suffix (`JR/SR/II/III/IV/VI`; single `I`/`V` excluded as ambiguous with middle initials) to the end, and folds a third comma segment as a suffix. Heuristic grounded in a 578-name live FEC sample; unit-locked in `tests/name-format.spec.js` (20 cases). **candidate.html carried an inline comma-reorder copy that bypassed the helper** (page title read "Joseph R Jr Biden") — de-inlined to `formatCandidateName`, locked by a suffix-name metadata-override test in candidate.spec.js. The "Marsha Blackburn" / "Joseph R Biden Jr" cases below all render correctly now. The historical diagnosis is preserved below for reference. **(Historical)** The helper (utils.js) split a FEC `"LAST, FIRST"` name on the comma and reordered to `"First Last"`, but did nothing with the honorific/suffix tokens the FEC embeds in those fields. Live examples now on `/races`: **"Marsha Mrs. Blackburn"**, **"Mark Dr. Green"**, **"Danny K. Mr. Davis"**, **"T. Jonathan Ossoff"**, **"Joseph R Jr Biden"**. It renders on the **seat-status line of any single-incumbent tile** whose incumbent's FEC name carries such a token (common in FEC data; open-seat and "Multiple incumbents" tiles show no name, so it's specifically the one-incumbent case). **Why this is more than cosmetic now:** the *old* /races browse row showed only a candidate count + total — **no names at all**. 2c introduced candidate names to /races (the `Incumbent: {name}` line), so a defect that was previously confined to the candidate / committee / race **detail** pages now renders on the primary **discovery** surface, across a large share of cards. That expansion is the reason to pick it up, not shelve it. **This also closes the banked "Incumbent: Biden on 2024 President" watch item:** the 2024 presidential tile reads **"Incumbent: Joseph R Jr Biden"** — *both* the surprising-incumbent quirk (Biden was the cycle's incumbent-of-record even though Harris became the nominee; race.html carries the who-actually-ran detail) *and* the mangling above, in one string. **Fix scope is site-wide, not races-only:** teach `toTitleCase` to strip/normalize honorifics (MR/MRS/DR/…) and handle suffixes (JR/SR/II/III) — it touches every candidate-name render, so validate against the detail pages in the same change.
...
**Caption A + office omission (Senate-not-up handling).** Why it matters: the border-ZIP "why does my neighbor state get a Senate race and I don't" confusion is the whole reason Caption A exists. **Expectations below updated 2026-07-16** — Caption A now renders in BOTH layouts (the single-state rows previously expected a silent drop; that was decision #3, reversed for the Senate).

| Input | Expected render | Tests | Status |
|---|---|---|---|
| `42223` @ 2024 | multi-state KY+TN; **Kentucky** group ends with **"No Kentucky Senate race in 2024"** (Caption A); **Tennessee** shows a real `Senate • TN` card | Caption A fires on a *truly-empty* Senate list; multi-state grouping; President ungrouped on top | ✅ |
| `30303` @ 2024 | single-state GA; `House • GA-05` + President; **list ends with "No Georgia Senate race in 2024"** (Caption A, flat layout) | Caption A in the flat layout — end-of-list placement mirrors the grouped `beforeend` | ✅ |
| `60629` @ 2024 | President + 4 House cards; **list ends with "No Illinois Senate race in 2024"** | flat Caption A alongside multi-district | ✅ |
| any single-state, Senate present | no note anywhere | Caption A is **Senate-only** — House never nulls; a null President is a data anomaly, not "not up" | ✅ |
...
**Seat-status contract (drives the card's right side).** Why: this is the copy users read on every tile; the collapse case is also Caption B's (dark) target. **Expectations updated 2026-07-16** — mark-the-exception: an incumbent race now shows NO seat signal (the rows below previously expected "Incumbent: {name}"), `open`/`multiple` are `.tag-neutral` chips, and "No candidates reported" became "No candidate filings".

| Input | Expected render | Tests | Status |
|---|---|---|---|
| `30303` @ 2024 | `House • GA-05` → **no seat signal at all**, total only; card carries `data-seat-kind="incumbent"` | single-incumbent → nothing rendered (the name is derived + carried, never shown) | ✅ |
| `30303` @ 2020 | `Senate • GA` → **"Multiple incumbents"** chip (Perdue + Loeffler collapse); President + House show totals only (both single-incumbent) | 2+ distinct incumbents → tag (Caption B target, ships dark) | ✅ |
| *(open-seat example — TBD verified)* | → **"Open seat"** chip | ≥1 candidate, 0 incumbents | behavioral |
| any House race with 0 filers | → **"No candidate filings"** plain text, no total | `none` → not a tag; $0 total suppressed | behavioral |
