...
- **Automated test specs for Stage 1 + 2a (test debt)** — both were verified via curl + node harnesses (run, not committed as specs), so CLAUDE.md's "new API behavior gets a `.spec.js` assertion" bar is currently **unmet** for the dormant Stage-1/2a code. Owe: a geo-resolver normalize/branch unit spec + a races-resolver pure-fn spec (sankey.spec.js pattern — `addScriptTag`, no DOM), landing with the 2e suite (geocod.io mock + races.spec.js flow + shared.spec.js races.html structural updates).
- **CLAUDE.md updates (deferred to Stage-2 completion)** — held mid-arc to avoid churn (the strategy doc is the interim source of truth). When the user-facing surface lands, add to CLAUDE.md: `functions/api/geo/[[path]].js` + `races-resolver.js` in Current files; the geo-resolver architecture (v2 `/geocode`, `cd{congress}`, cache-on-miss + `GEO_CACHE` binding, typed errors); the FEC race layer + seat-status contract; and the location-search replacement of the races.html browse (+ ia.md URL patterns, design-system race-tile card).
