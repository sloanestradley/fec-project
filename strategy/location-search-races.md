...
**Banked but NOT 2e (separate follow-ons — see "Known defects" below):** the `/elections/search/` contest-existence gate (phantom races); ~~`toTitleCase` honorific/suffix mangling (site-wide)~~ **RESOLVED 2026-07-15** (see Known defects entry); the `?year=` deep-link clamp/canonicalize; the optional `GEO_CACHE` KV production binding.
