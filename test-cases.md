...
- [ ] **Mega-committee empty state (committees and conduits):** viewing ActBlue (C00401224) on a specific cycle shows "Unable to show due to high transaction volume." in both `#committee-donors-tbody` and `#conduits-tbody` (both hit the 100-page adaptive gate; top_committees KV is temporarily disabled pending cm.txt work, so committees always fall through to the Schedule A fallback)
...
- [ ] **Top Committee Contributors — API fallback (specific cycle, small committee):** Marie for Congress on a specific cycle shows committee contributor rows via API fallback; data note has the "complete 2023–2024 cycle, deduplicated by committee ID" sentence. (KV path temporarily disabled; all committees currently use the Schedule A fallback.)
- [ ] **Aggregations endpoint handshake:** on cycle switch, Network tab shows a `GET /api/aggregations/top-contributors` request with a 200 response and `{results, source}` body. (A `top-committees` request also fires but always returns `{results:null, source:'api'}` under the current feature flag.)
