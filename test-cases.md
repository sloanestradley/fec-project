...
**Automated tests (Track 1):** Run `npx playwright test` from the project root before and after changes. 417 structural tests across all pages run in ~2 minutes with mocked API. See `TESTING.md` for full details.
...
- [ ] Meta-row contains, in order: party tag → `FEC ID · {candidate_id}` neutral tag → `First filed YYYY` inline prose (IBM Plex Serif 0.875rem) → incumbent tag (when applicable)
...
| 2026-04-23 | Header polish pass — candidate.html + committee.html. Structural restack only; no behavioral change to modal, cycle switcher, or tabs. (1) DOM: `.meta-row` moved OUT of `.profile-header-row` to be a sibling below it inside `.page-header` on both pages. (2) New content in `.meta-row`: `<span class="tag tag-neutral fec-id-tag">FEC ID · {id}</span>` tag on both pages + `<span class="meta-prose">First filed YYYY</span>` (candidate) or `Active since YYYY` (committee, **conditional**: omitted entirely when `filing_frequency === 'T'` or `'A'` — terminated committees render just the FEC ID tag, no prose). Year parsed from `first_file_date` ISO string via `.slice(0,4)`. FEC field verified live on both `/candidate/{id}/` and `/committee/{id}/` endpoints before coding. (3) New `.meta-prose` utility class in `styles.css` (IBM Plex Serif 0.875rem 400, `color:var(--subtle)`, `line-height:var(--lh-normal)`) — reuses the existing `prose` named type style, no new tokens. Added to the `prose` specimen's usage list in `design-system.html`. (4) `.committees-link` restyled to match `.button-group-btn.active` navy-filled treatment. The three color/border declarations are shared via a comma-grouped selector `.button-group-btn.active, .committees-link { ... }`; sizing/font rules stay per-component. Design-system note explicitly documents the navy fill as the site's shared language for "selected toggle segment" AND "primary action on this page." Redundant `.committees-link` override inside `design-system.html` inline `<style>` removed — production and demo now share one rule. Button-group card status promoted `feed-only` → `stable`; demo panel gained a `.committees-link` example row. (5) New `.page-header > .meta-row { margin-top: var(--space-4) }` scoped rule provides the 4px gap between the title row and the new meta-row. (6) Mock fixtures extended: `first_file_date: '2022-02-22'` added to CANDIDATE fixture, `first_file_date: '2020-04-24'` to COMMITTEE fixture in `tests/helpers/api-mock.js` — new assertions depend on these. (7) Compact sticky header unchanged — existing `#profile-header.compact .meta-row { display:none }` and `#committee-header.compact .meta-row { display:none }` selectors hide the new meta-row automatically; `#profile-header.compact #committees-trigger { display:none !important }` still applies to the restyled trigger. (8) Playwright: 8 new assertions across `tests/candidate.spec.js` (FEC ID tag text, First filed prose, structural sibling check, refined `.tag-neutral:not(.incumbent-tag):not(.fec-id-tag)` filter on the pre-existing race-tag-absent assertion) and `tests/pages.spec.js` (committee FEC ID + Active since + structural sibling check, plus a new `committee.html — terminated committee` describe block that routes over just `/committee/{id}/` to flip `filing_frequency` to `'T'` and asserts `.meta-prose` count = 0 while `.fec-id-tag` still renders). | candidate.html, committee.html, styles.css, design-system.html, tests/helpers/api-mock.js, tests/candidate.spec.js, tests/pages.spec.js, CLAUDE.md, test-cases.md, claude-to-claude.md | **Visual verification gap:** Playwright validates DOM structure and text content but cannot verify visual spacing (is 4px the right gap? does the navy button look right?), mobile-viewport wrap behavior on profile pages (≤860px), or terminated-committee rendering in a live browser. Sloane to eyeball on the live deploy after push. No known issues beyond this. | 425/425 Track 1 passing (+8 new assertions, was 417) |
