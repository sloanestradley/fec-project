...
- [ ] "Top Individual Contributors" card renders below Top Conduit Sources with at least one row; header uses the latest 2-year period (House: full cycle; Senate: just the most recent sub-cycle, e.g. `· 2025–2026` on Gillibrand 2026)
- [ ] Senate/Presidential only: data note includes "Individual contributors reflect the latest 2-year reporting period (…) only; totals above aggregate the full election cycle."
- [ ] KV-hit case: data note includes "Top individual contributors pre-computed from FEC bulk data, refreshed daily."
- [ ] KV-miss case (mega-committee principal): tbody shows "Data not available for this committee."
...
- [ ] **KV miss + mega-committee:** a committee above PAGE_THRESHOLD without a KV entry shows `"Data not available for this committee."` in `#individual-donors-tbody`
...
- [ ] **KV-backed Top Committee Contributors (specific cycle):** ActBlue (C00401224) on 2024 shows real committee contributors (committee names, non-zero totals) instead of the "Unable to show" empty state; data note contains "Top committee contributors pre-computed from FEC bulk data (pas2), refreshed daily."
- [ ] **API-fallback Top Committee Contributors (specific cycle, small committee):** Marie for Congress on a specific cycle shows committee contributor rows via API fallback; data note has the "complete 2023–2024 cycle, deduplicated by committee ID" sentence
- [ ] **Top Conduit Sources still conditional:** on a mega-committee with a KV hit for committees, the Top Committee Contributors table is populated from bulk BUT the Top Conduit Sources table still shows "Unable to show conduit sources due to high transaction volume." (decoupled from committees now)
- [ ] **Aggregations endpoint handshake:** on cycle switch, Network tab shows exactly two `GET /api/aggregations/*` requests (top-contributors AND top-committees) with 200 responses and `{results, source}` bodies
