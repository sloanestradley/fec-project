...
- [ ] Meta-row contains, in order: **race link** (`.candidate-race-label` — T15 relocated from above title; reads e.g. "US HOUSE: WASHINGTON'S 3RD DISTRICT" in red-700 Oswald 400 uppercase; `<a>` to `/race?state=WA&district=03&office=H&year=...`) → party tag → incumbent tag (when candidate is the incumbent for the active cycle) → `FEC ID · {candidate_id}` neutral tag
- [ ] Race link visual mismatch acknowledged: 1.25rem Oswald next to 0.625rem mono tags reads as oversized. Pending explicit direction on a meta-row context variant — visible in production for evaluation.
