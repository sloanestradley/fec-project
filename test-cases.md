...
| 2026-04-28 | **T9 + T10 — committee.html parity arc closes (in-place transitions).** **T9 (commit `8ec232b`):** Lifted candidate.html's `switchView()` into `initViewSwitcher(config)` factory in `utils.js` (~110 lines). Returns `{ switchTo, claimToken, isCurrentToken }`. Single-counter token namespace exposed via API so the page's own loadCycle can participate (separate counters wouldn't invalidate each other's in-flight fetches). Cache stays per-page (T13-friendly). Helper compact-naive — `getDetailScrollTarget(indexScrollY)` callback closes over the page's compactThreshold. Candidate.html refactored to consume helper: deleted `currentFetchId` and inline `indexScrollY` module vars; `loadCycle` uses `view.claimToken()`/`view.isCurrentToken()`. **T10 (commit `0eef3df`):** Committee.html adopts the helper. Module vars added (`cachedMeta`, `cachedAllTotalsRes`, `compactThreshold`, `compactHeaderInited`, `cycleFetchToken`, `view`). `Promise.all` upfront extracted into `fetchIndexData()` with per-page cache. Compact-header IIFE replaced with named `initCompactHeader()` exposing compactThreshold. `fetchAndRenderAssocSection()` moved out of detail-view branch to init-unconditional (in-place cycle clicks don't re-run init). Cycle-switcher onchange unchanged — token claim happens automatically inside renderStats. `cycleFetchToken = view.claimToken()` claimed at top of renderStats; checked in `renderRaisedIfReady` / `renderSpentIfReady` after `await raisedDataPromise`/`spentDataPromise` — race protection for Schedule A/B fetches. Bottom-of-script `window.location.reload()` hashchange listener deleted; replaced with `view.switchTo()` listener inside init. Plus parity polish: mirrored the `compactThreshold > 0` guard inline comment back to candidate.html (pure docs, no behavior change). Tests: 8 new in-place transition assertions on committee.spec.js mirroring candidate.spec.js's describe block (no-reload, DOM identity, back-cache hit, scroll behavior, compact continuity, rapid cycle hash). All existing 72 committee tests pass without modification under the refactor — the cycle-row-click test was observably equivalent under T8 (reload) and T10 (in-place). | candidate.html, committee.html, utils.js, tests/committee.spec.js, CLAUDE.md, ia.md, TESTING.md, test-cases.md | Manual verification block from prompt covered by automated in-place transition tests in committee.spec.js + the same block in candidate.spec.js (T6.5). Live browser checks against `/committee/C00806174` (Marie) and `/committee/C00010603` (DCCC long history) outstanding at session end — recommended before next session if anything in user-visible behavior feels off. | 484/484 Track 1 passing (+8 new committee in-place transition tests, was 476) |
